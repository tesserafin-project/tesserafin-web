 ...
 *
 * Reads every theme directory under `reefin-design/themes/<id>/` (a `theme.json` + `tokens.json`
 * pair, RFC-0005 §7.3), validates both against `reefin-design/schema/*.schema.json`, and emits the
 ...
 *     scoping rule).
 *   - `src/ui/tokens/<themeId>.ts` — the same tokens as a typed `ReefinTokens` object
 ...
 *
 * Usage:
 *   node reefin-design/scripts/generate-web-tokens.mjs
...
const REEFIN_DESIGN_DIR = resolve(__dirname, '..');
const REPO_ROOT = resolve(REEFIN_DESIGN_DIR, '..');
const THEMES_DIR = join(REEFIN_DESIGN_DIR, 'themes');
const SCHEMA_DIR = join(REEFIN_DESIGN_DIR, 'schema');
 ...
 * Source: reefin-design/themes/${themeSlugFromId(themeId)}/{theme,tokens}.json
...
`;
    const body = `export const ${constName}: ReefinTokens = ${jsonLiteral(tokens, 0)};
...
function themeSlugFromId(themeId) {
    // Directory name under reefin-design/themes/, independent of the manifest's dotted "id".
