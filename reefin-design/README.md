# reefin-design (provisional name)

Cross-platform design token socle for Reefin, specified in `docs/reefin/RFC-0005-design-system-
...
> The name `reefin-design` is provisional (RFC-0005 §3.2 explicitly flags it as such). It has not
...
- `src/ui/tokens/<themeId>.ts` — the same tokens as a `ReefinTokens` object (typed against the
...
`reefin-design/scripts/validate-schema.mjs` is a small, purpose-built JSON Schema validator
