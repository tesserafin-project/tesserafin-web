...
const importController = (
    ...
    switch (appType) {
        case AppType.Dashboard:
            return Promise.all([
                import(
                    /* webpackChunkName: "[request]" */ `../../apps/dashboard/controllers/${controller}`
                ),
                import(
                    /* webpackChunkName: "[request]" */ `../../apps/dashboard/controllers/${view}`
                ).then((html) => globalize.translateHtml(html))
            ]);
        case AppType.Wizard:
            return Promise.all([
                import(
                    /* webpackChunkName: "[request]" */ `../../apps/wizard/controllers/${controller}`
                ),
                import(
                    /* webpackChunkName: "[request]" */ `../../apps/wizard/controllers/${view}`
                ).then((html) => globalize.translateHtml(html))
            ]);
        default:
            return Promise.all([
                import(
                    /* webpackChunkName: "[request]" */ `../../apps/legacy/controllers/${controller}`
                ),
                import(
                    /* webpackChunkName: "[request]" */ `../../apps/legacy/controllers/${view}`
                ).then((html) => globalize.translateHtml(html))
            ]);
    }
};
...
const loadView = async (
    ...
    const [controllerFactory, viewHtml] = await importController(
        appType,
        controller,
        view
    );
            ...
            const loadPage = () => {
                ...
                return viewManager
                    ...
                    .catch(async (result?: RestoreViewFailResponse) => {
                        if (!result?.cancelled) {
                            console.debug(
                                '[ViewManagerPage] restore failed; loading view [%s]',
                                view
                            );
                            return loadView(
                                appType,
