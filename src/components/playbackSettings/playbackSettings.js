...
} from 'apps/legacy/features/playback/utils/mediaSegmentSettings';
...
import {
    ID as STILL_WATCHING_ID,
    StillWatchingConfiguration,
    StillWatchingOptions
} from 'plugins/stillWatching/constants';
...
function fillStillWatchingOptions(select, selectedOption) {
    select.innerHTML = Object.values(StillWatchingOptions)
        .map((value) => {
            let label;
            if (value === StillWatchingOptions.Disabled) {
                label = globalize.translate('Disabled');
            } else {
                const { count, duration: durationMs } =
                    StillWatchingConfiguration[value];
                const duration = Math.round(
                    durationMs / MILLISECONDS_PER_MINUTE
                );
...
function loadForm(context, user, userSettings, systemInfo, apiClient) {
    ...
    const stillWatchingPlugin = pluginManager
        .ofType(PluginType.PreplayIntercept)
        .some((p) => p.id === STILL_WATCHING_ID);
    context
        .querySelector('.fldStillWatchingOption')
        .classList.toggle('hide', !stillWatchingPlugin);
    fillStillWatchingOptions(
        context.querySelector('.selectStillWatchingOption'),
        userSettings.stillWatchingPrompt()
    );
...
function saveUser(context, user, userSettingsInstance, apiClient) {
    ...
    userSettingsInstance.stillWatchingPrompt(
        context.querySelector('.selectStillWatchingOption').value
    );
