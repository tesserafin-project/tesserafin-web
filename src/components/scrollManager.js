    ...
    if (element) {
        ...
        while (parent && parent !== document.body) {
            ...
            if (
                overflow === 'scroll' ||
                (overflow === 'auto' &&
                    parent[scrollerHint.nameScroll] >
                        parent[scrollerHint.nameClient])
            ) {
                return parent;
            }
...
function calcScroll(scrollerData, elementPos, elementSize, centered) {
    ...
    if (centered) {
        ...
        const delta = fitRange(
            elementPos,
            elementPos + elementSize - 1,
            scrollerData.scrollPos,
            scrollerData.scrollPos + scrollerData.clientSize - 1
        );
...
function scrollToHelper(scroller, options) {
    if ('scrollTo' in scroller) {
        if (!supportsScrollToOptions) {
            const scrollX =
                options.left !== undefined ? options.left : scroller.scrollLeft;
            const scrollY =
                options.top !== undefined ? options.top : scroller.scrollTop;
...
function builtinScroll(xScroller, scrollX, yScroller, scrollY, smooth) {
    ...
    if (xScroller !== yScroller) {
        if (xScroller) {
            scrollToHelper(xScroller, {
                left: scrollX,
                behavior: scrollBehavior
            });
        }
        if (yScroller) {
            scrollToHelper(yScroller, {
                top: scrollY,
                behavior: scrollBehavior
            });
        }
...
export function scrollTo(scrollX, scrollY, smooth) {
    ...
    scrollX = clamp(
        Math.round(scrollX),
        0,
        xScrollerData.scrollSize - xScrollerData.clientSize
    );
    scrollY = clamp(
        Math.round(scrollY),
        0,
        yScrollerData.scrollSize - yScrollerData.clientSize
    );
...
export function scrollToElement(element, smooth) {
    ...
    const isFixed =
        ...
        (!offsetParent.offsetParent ||
            window.getComputedStyle(offsetParent).position === 'fixed');
    ...
    if (
        xScroller === yScroller &&
        (xScrollerData.custom || yScrollerData.custom)
    ) {
        return;
    }
