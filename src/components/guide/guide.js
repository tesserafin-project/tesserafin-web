...
function updateProgramCellOnScroll(cell, scrollPct) {
    ...
    if (guideProgramName) {
        if (pctOfWidth > 0 && pctOfWidth <= 100) {
            guideProgramName.style.transform =
                'translateX(' + pctOfWidth + '%)';
...
function updateProgramCellsOnScroll(programGrid, programCells) {
    ...
    requestAnimationFrame(function () {
        ...
        const scrollPct = scrollLeft
            ? (scrollLeft / programGrid.scrollWidth) * 100
            : 0;
...
function onProgramGridClick(e) {
    ...
    if (programCell) {
        ...
        startDate = datetime
            .parseISO8601Date(startDate, { toLocal: true })
            .getTime();
        endDate = datetime
            .parseISO8601Date(endDate, { toLocal: true })
            .getTime();
...
function Guide(options) {
    ...
    function reloadGuide(
        ...
        const displayMovieContent =
            !categories.length || categories.indexOf('movies') !== -1;
        const displaySportsContent =
            !categories.length || categories.indexOf('sports') !== -1;
        const displayNewsContent =
            !categories.length || categories.indexOf('news') !== -1;
        const displayKidsContent =
            !categories.length || categories.indexOf('kids') !== -1;
        const displaySeriesContent =
            !categories.length || categories.indexOf('series') !== -1;
        ...
        const renderOptions = {
            showHdIcon:
                allowIndicators &&
                userSettings.get('guide-indicator-hd') === 'true',
            showLiveIndicator:
                allowIndicators &&
                userSettings.get('guide-indicator-live') !== 'false',
            showPremiereIndicator:
                allowIndicators &&
                userSettings.get('guide-indicator-premiere') !== 'false',
            showNewIndicator:
                allowIndicators &&
                userSettings.get('guide-indicator-new') !== 'false',
            showRepeatIndicator:
                allowIndicators &&
            ...
            .then(function (channelsResult) {
                ...
                if (channelsResult.TotalRecordCount > channelLimit) {
                    ...
                    if (channelQuery.StartIndex) {
                        context.querySelector('.btnPreviousPage').disabled =
                            false;
                    } else {
                        context.querySelector('.btnPreviousPage').disabled =
                            true;
                    ...
                    if (
                        ...
                        channelsResult.TotalRecordCount
                    ) {
                ...
                apiClient
                    .getLiveTvPrograms(programQuery)
                    .then(function (programsResult) {
                        ...
                        renderGuide(
                            context,
                            date,
                            channelsResult.Items,
                            programsResult.Items,
                            renderOptions,
                            guideOptions,
                            apiClient
                        );
    ...
    function parseDates(program) {
        if (!program.StartDateLocal) {
            try {
                program.StartDateLocal = datetime.parseISO8601Date(
                    program.StartDate,
                    { toLocal: true }
                );
            } catch (err) {
        ...
        if (!program.EndDateLocal) {
            try {
                program.EndDateLocal = datetime.parseISO8601Date(
                    program.EndDate,
                    { toLocal: true }
                );
            } catch (err) {
    ...
    function getChannelProgramsHtml(
        ...
        const outerCssClass = layoutManager.tv
            ? 'channelPrograms channelPrograms-tv'
            : 'channelPrograms';
        ...
        const clickAction = layoutManager.tv
            ? ItemAction.Link
            : ItemAction.ProgramDialog;
        ...
        const displaySeriesContent =
            !categories.length || categories.indexOf('series') !== -1;
        const enableColorCodedBackgrounds =
        ...
        for (
            let i = listInfo.startIndex, length = programs.length;
            i < length;
            i++
        ) {
            ...
            if (program.SeriesTimerId) {
                timerAttributes +=
                    ' data-seriestimerid="' + program.SeriesTimerId + '"';
            }
            ...
            if (displayInnerContent) {
                ...
                html +=
                    '<div class="guide-programNameCaret hide"><span class="guideProgramNameCaretIcon material-icons keyboard_arrow_left" aria-hidden="true"></span></div>';

                html +=
                    '<div class="guideProgramNameText">' +
                    escapeHtml(program.Name);

                let indicatorHtml = null;
                if (program.IsLive && programOptions.showLiveIndicator) {
                    indicatorHtml =
                        '<span class="liveTvProgram guideProgramIndicator">' +
                        globalize.translate('Live') +
                        '</span>';
                } else if (
                    program.IsPremiere &&
                    programOptions.showPremiereIndicator
                ) {
                    indicatorHtml =
                        '<span class="premiereTvProgram guideProgramIndicator">' +
                        globalize.translate('Premiere') +
                        '</span>';
                } else if (
                    program.IsSeries &&
                    !program.IsRepeat &&
                    programOptions.showNewIndicator
                ) {
                    indicatorHtml =
                        '<span class="newTvProgram guideProgramIndicator">' +
                        globalize.translate('New') +
                        '</span>';
                } else if (
                    program.IsSeries &&
                    program.IsRepeat &&
                    programOptions.showRepeatIndicator
                ) {
                    indicatorHtml =
                        '<span class="repeatTvProgram guideProgramIndicator">' +
                ...
                if (program.EpisodeTitle && programOptions.showEpisodeTitle) {
                    ...
                    if (
                        ...
                        programOptions.showEpisodeTitle
                    ) {
                        html +=
                            '<span class="programSecondaryTitle">' +
                            escapeHtml(program.EpisodeTitle) +
                            '</span>';
                    }
                    html += '</div>';
                }
                ...
                if (program.IsHD && programOptions.showHdIcon) {
                    if (layoutManager.tv) {
                        html +=
                            '<div class="programIcon guide-programTextIcon guide-programTextIcon-tv">HD</div>';
                    } else {
                        html +=
                            '<div class="programIcon guide-programTextIcon">HD</div>';
                    }
                }
    ...
    function renderChannelHeaders(context, channels, apiClient) {
        ...
        for (const channel of channels) {
            ...
            if (hasChannelImage) {
                ...
                html +=
                    '<div class="guideChannelImage lazy" data-src="' +
                    url +
                    '"></div>';
            }
    ...
    function renderGuide(
        ...
        programs.sort(function (a, b) {
            return (
                getProgramSortOrder(a, channels) -
                getProgramSortOrder(b, channels)
            );
        });

        const activeElement = document.activeElement;
        const itemId = activeElement?.getAttribute
            ? activeElement.getAttribute('data-id')
            : null;
        let channelRowId = null;

        if (activeElement) {
            channelRowId = dom.parentWithClass(
                activeElement,
                'channelPrograms'
            );
            channelRowId = channelRowId?.getAttribute
                ? channelRowId.getAttribute('data-channelid')
                : null;
        }
        ...
        context.querySelector('.timeslotHeaders').innerHTML =
            getTimeslotHeadersHtml(startDate, endDate);
        items = {};
        ...
        if (guideOptions.focusProgramOnRender) {
            focusProgram(
                ...
                itemId,
                channelRowId,
                guideOptions.focusToTimeMs,
                guideOptions.startTimeOfDayMs
            );
        }

        scrollProgramGridToTimeMs(
            context,
            guideOptions.scrollToTimeMs,
            guideOptions.startTimeOfDayMs
        );
    }

    function scrollProgramGridToTimeMs(
        context,
        scrollToTimeMs,
        startTimeOfDayMs
    ) {
    ...
    function focusProgram(
        ...
        if (focusElem) {
            ...
            if (channelRowId) {
                autoFocusParent = context.querySelector(
                    '[data-channelid="' + channelRowId + '"]'
                );
            }
    ...
    function getDateTabText(date, isActive, tabIndex) {
        const cssClass = isActive
            ? 'emby-tab-button guide-date-tab-button emby-tab-button-active'
            : 'emby-tab-button guide-date-tab-button';

        let html =
            '<button is="emby-button" class="' +
            cssClass +
            '" data-index="' +
            tabIndex +
            '" data-date="' +
        ...
        switch (e.detail.command) {
            case 'up':
                if (programCell) {
                    ...
                    channelPrograms = dom.parentWithClass(
                        programCell,
                        'channelPrograms'
                    );
                    ...
                    if (newRow) {
                        focusableElements =
                            getChannelProgramsFocusableElements(newRow);
            ...
            case 'down':
                ...
                break;
            case 'left':
                container = programCell
                    ? dom.parentWithClass(programCell, 'channelPrograms')
                    : null;
    ...
    function onScrollerFocus(e) {
        ...
        if (lastFocusDirection === 'left') {
            ...
            lastFocusDirection === 'up' ||
            lastFocusDirection === 'down'
        ) {
            const verticalScroller = dom.parentWithClass(
                target,
                'guideVerticalScroller'
            );
            if (verticalScroller) {
                const focusedElement =
                    programCell || dom.parentWithTag(target, 'BUTTON');
    ...
    function setScrollEvents(view, enabled) {
        if (layoutManager.tv) {
            const guideVerticalScroller = view.querySelector(
                '.guideVerticalScroller'
            );
    ...
    function onTimerCreated(data) {
        ...
        const cells = options.element.querySelectorAll(
            '.programCell[data-id="' + programId + '"]'
        );
        for (const cell of cells) {
            const icon = cell.querySelector('.timerIcon');
            if (!icon) {
                cell.querySelector('.guideProgramName').insertAdjacentHTML(
                    'beforeend',
                    '<span class="timerIcon material-icons programIcon fiber_manual_record"></span>'
                );
            }
    ...
    function onSeriesTimerCancelled(data) {
        ...
        // find guide cells by timer id, remove timer icon
        const cells = options.element.querySelectorAll(
            '.programCell[data-seriestimerid="' + id + '"]'
        );
    ...
    if (layoutManager.tv) {
        dom.addEventListener(
            guideContext.querySelector('.guideVerticalScroller'),
            'focus',
            onScrollerFocus,
            {
                capture: true,
    ...
    if (browser.iOS || browser.osx) {
        guideContext
            .querySelector('.channelsContainer')
            .classList.add('noRubberBanding');
    ...
    guideContext
        .querySelector('.btnGuideViewSettings')
        .addEventListener('click', function () {
    ...
    guideContext
        .querySelector('.guideDateTabs')
        .addEventListener('tabchange', function (e) {
            const allTabButtons = e.target.querySelectorAll(
                '.guide-date-tab-button'
            );

            const tabButton =
                allTabButtons[parseInt(e.detail.selectedTabIndex, 10)];
            if (tabButton) {
                const previousButton =
                    e.detail.previousIndex == null
                        ? null
                        : allTabButtons[parseInt(e.detail.previousIndex, 10)];
                ...
                if (scrollWidth) {
                    scrollToTimeMs =
                        (programGrid.scrollLeft / scrollWidth) * msPerDay;
                } else {
                    scrollToTimeMs = 0;
                }
                ...
                if (previousButton) {
                    ...
                    previousDate.setTime(
                        parseInt(previousButton.getAttribute('data-date'), 10)
                    );
