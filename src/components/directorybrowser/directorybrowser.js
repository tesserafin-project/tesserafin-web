...
function refreshDirectoryBrowser(page, path, fileOptions, updatePathOnError) {
    ...
    Promise.all(promises).then(
        (responses) => {
            ...
            const parentPath =
                (responses[1] ? JSON.parse(responses[1]) : '') || '';
            let html = '';
...
function getItem(cssClass, type, path, name) {
    ...
    html += '</div>';
    html +=
        '<span class="material-icons arrow_forward" aria-hidden="true" style="font-size:inherit;"></span>';
    html += '</div>';
    return html;
}
...
function getEditorHtml(options) {
    ...
    if (!readOnlyAttribute) {
        html +=
            '<div class="results paperList" style="max-height: 200px; overflow-y: auto;"></div>';
    }
...
function initEditor(content, options, fileOptions) {
    ...
    content.addEventListener('change', (e) => {
        ...
        if (
            txtDirectoryPickerPath &&
            txtDirectoryPickerPath.id === 'txtDirectoryPickerPath'
        ) {
            refreshDirectoryBrowser(
                content,
                txtDirectoryPickerPath.value,
                fileOptions
            );
        }
    });
...
function getDefaultPath(options) {
    if (options.path) {
        ...
        return ApiClient.getJSON(
            ApiClient.getUrl('Environment/DefaultDirectoryBrowser')
        ).then(
            (result) => {
                return result.Path || '';
            },
    ...
    show = (options) => {
        ...
        getDefaultPath(options).then((fetchedInitialPath) => {
            ...
            html +=
                ...
                globalize.translate('HeaderSelectPath');
            html += '</h3>';
            html += '</div>';
            ...
            dlg.querySelector('.btnCloseDialog').addEventListener(
                'click',
                () => {
            ...
            dlg.querySelector('#txtDirectoryPickerPath').value =
                fetchedInitialPath;
