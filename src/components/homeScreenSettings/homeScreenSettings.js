...
function getPerLibrarySettingsHtml(item, user, userSettings) {
    const collectionType =
        item.Type === 'CollectionFolder' && item.CollectionType == null
            ? 'mixed'
            : item.CollectionType;
    ...
    if (landingScreenTypes.includes(collectionType)) {
        const idForLanding =
            collectionType === 'livetv' ? collectionType : item.Id;
...
function loadForm(context, user, userSettings, apiClient) {
    ...
    const promise1 = queryClient.fetchQuery(
        getUserViewsQuery(ServerConnections.getApi(apiClient.serverId()), {
            userId: user.Id,
            includeHidden: true
        })
    );
