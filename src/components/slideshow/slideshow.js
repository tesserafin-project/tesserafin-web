...
export default function (options) {
    ...
    function createElements(slideshowOptions) {
        ...
        html +=
            '<div class="slideshowSwiperContainer"><div class="swiper-wrapper"></div></div>';
        ...
        if (slideshowOptions.interactive && !layoutManager.tv) {
            ...
            if (actionButtonsOnTop) {
                html += getIcon(
                    'play_arrow',
                    'btnSlideshowPause slideshowButton',
                    true
                );
            ...
            html += getIcon(
                'close',
                'slideshowButton btnSlideshowExit hide-mouse-idle-tv',
                false
            );
            html += '</div>';
            ...
            html +=
                '<div class="slideshowImage"></div><h1 class="slideshowImageText"></h1>';
        }
        ...
        if (slideshowOptions.interactive && !layoutManager.tv) {
            dialog
                .querySelector('.btnSlideshowExit')
            ...
            if (btnFullscreen) {
                btnFullscreen.addEventListener(
                    'click',
                    getClickHandler(fullscreen)
                );
            }

            const btnFullscreenExit =
                dialog.querySelector('.btnFullscreenExit');
            if (btnFullscreenExit) {
                btnFullscreenExit.addEventListener(
                    'click',
                    getClickHandler(fullscreenExit)
                );
            }
        ...
        const btnSlideshowPrevious = dialog.querySelector(
    ...
    function onAutoplayStart() {
        const btnSlideshowPause = dialog.querySelector(
            '.btnSlideshowPause .material-icons'
        );
    ...
    function onZoomChange(swiper, scale, imageEl, slideEl) {
        ...
        if (zoomImage) {
            ...
            if (scale > 1) {
                if (
                    zoomImage.classList.contains('swiper-zoom-fakeimg-hidden')
                ) {
        ...
        import('swiper/bundle').then(({ Swiper }) => {
            swiperInstance = new Swiper(
                dialogElement.querySelector('.slideshowSwiperContainer'),
                {
                    ...
                    autoplay:
                        swiperOptions.autoplay ?? !swiperOptions.interactive,
                    keyboard: {
                        enabled: true
                    },
                    preloadImages: true,
    ...
    function getSwiperSlideHtmlFromSlide(item) {
        ...
        html +=
            '<img src="' + item.originalImage + '" class="swiper-slide-img">';
        html += '</div>';
    ...
    function slideToShow(element, slideFrom) {
        ...
        const onFinish = function () {
            const btnSlideshowPause =
                element.querySelector('.btnSlideshowPause');
    ...
    function onPointerMove(event) {
        const pointerType =
            event.pointerType || (layoutManager.mobile ? 'touch' : 'mouse');
