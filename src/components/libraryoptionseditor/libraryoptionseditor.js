...
function populateLanguages(parent) {
    return ApiClient.getCultures().then((languages) => {
        populateLanguagesIntoSelect(
            parent.querySelector('#selectLanguage'),
            languages
        );
        populateLanguagesIntoList(
            parent.querySelector('.subtitleDownloadLanguages'),
            languages
        );
    });
}
...
function renderMetadataReaders(page, plugins) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        ...
        html +=
            '<span class="listItemIcon material-icons live_tv" aria-hidden="true"></span>';
...
function getMetadataFetchersForTypeHtml(
    availableTypeOptions,
    libraryOptionsForType
) {
    let html = '';
    let plugins = availableTypeOptions.MetadataFetchers;

    plugins = getOrderedPlugins(
        plugins,
        libraryOptionsForType.MetadataFetcherOrder || []
    );
    if (!plugins.length) return html;

    html +=
        '<div class="metadataFetcher" data-type="' +
    ...
    html +=
        ...
        globalize.translate(
            ...
            globalize.translate('TypeOptionPlural' + availableTypeOptions.Type)
        ) +
        '</h3>';
    html += '<div class="checkboxList paperList checkboxList-paperList">';
    ...
    plugins.forEach((plugin, index) => {
        html +=
            '<div class="listItem metadataFetcherItem sortableOption" data-pluginname="' +
            escapeHtml(plugin.Name) +
            '">';
        const isChecked = libraryOptionsForType.MetadataFetchers
            ? libraryOptionsForType.MetadataFetchers.includes(plugin.Name)
            : plugin.DefaultEnabled;
        const checkedHtml = isChecked ? ' checked="checked"' : '';
        html +=
            '<label class="listItemCheckboxContainer"><input type="checkbox" is="emby-checkbox" class="chkMetadataFetcher" data-pluginname="' +
            escapeHtml(plugin.Name) +
            '" ' +
            checkedHtml +
            '><span></span></label>';
        ...
        if (index > 0) {
            html +=
                '<button type="button" is="paper-icon-button-light" title="' +
                globalize.translate('Up') +
                '" class="btnSortableMoveUp btnSortable" data-pluginindex="' +
                index +
                '"><span class="material-icons keyboard_arrow_up" aria-hidden="true"></span></button>';
        } else if (plugins.length > 1) {
            html +=
                '<button type="button" is="paper-icon-button-light" title="' +
                globalize.translate('Down') +
                '" class="btnSortableMoveDown btnSortable" data-pluginindex="' +
                index +
...
function renderMetadataFetchers(page, availableOptions, libraryOptions) {
    ...
    for (const availableTypeOptions of availableOptions.TypeOptions) {
        html += getMetadataFetchersForTypeHtml(
            availableTypeOptions,
            getTypeOptions(libraryOptions, availableTypeOptions.Type) || {}
        );
    }
    elem.innerHTML = html;
    if (html) {
...
function renderSubtitleFetchers(page, availableOptions, libraryOptions) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        ...
        const isChecked = libraryOptions.DisabledSubtitleFetchers
            ? !libraryOptions.DisabledSubtitleFetchers.includes(plugin.Name)
            : plugin.DefaultEnabled;
        const checkedHtml = isChecked ? ' checked="checked"' : '';
...
function renderLyricFetchers(page, availableOptions, libraryOptions) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        ...
        const isChecked = libraryOptions.DisabledLyricFetchers
            ? !libraryOptions.DisabledLyricFetchers.includes(plugin.Name)
            : plugin.DefaultEnabled;
        const checkedHtml = isChecked ? ' checked="checked"' : '';
...
function renderMediaSegmentProviders(page, availableOptions, libraryOptions) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        ...
        const isChecked = libraryOptions.DisabledMediaSegmentProviders
            ? !libraryOptions.DisabledMediaSegmentProviders.includes(
                  plugin.Name
              )
            : plugin.DefaultEnabled;
        const checkedHtml = isChecked ? ' checked="checked"' : '';
...
function getImageFetchersForTypeHtml(
    availableTypeOptions,
    libraryOptionsForType
) {
    let html = '';
    ...
    plugins = getOrderedPlugins(
        plugins,
        libraryOptionsForType.ImageFetcherOrder || []
    );
    if (!plugins.length) return html;

    html +=
        '<div class="imageFetcher" data-type="' +
        availableTypeOptions.Type +
        '">';
    html +=
        '<div class="flex align-items-center" style="margin:1.5em 0 .5em;">';
    html +=
        '<h3 class="checkboxListLabel" style="margin:0;">' +
        globalize.translate(
    ...
    if (
        supportedImageTypes.length > 1 ||
        (supportedImageTypes.length === 1 &&
            supportedImageTypes[0] !== 'Primary')
    ) {
        html +=
            '<button is="emby-button" class="raised btnImageOptionsForType" type="button" style="font-size:90%;"><span>' +
    ...
    for (let i = 0; i < plugins.length; i++) {
        const plugin = plugins[i];
        html +=
            '<div class="listItem imageFetcherItem sortableOption" data-pluginname="' +
            escapeHtml(plugin.Name) +
            '">';
        const isChecked = libraryOptionsForType.ImageFetchers
            ? libraryOptionsForType.ImageFetchers.includes(plugin.Name)
            : plugin.DefaultEnabled;
        const checkedHtml = isChecked ? ' checked="checked"' : '';
        html +=
            '<label class="listItemCheckboxContainer"><input type="checkbox" is="emby-checkbox" class="chkImageFetcher" data-pluginname="' +
            escapeHtml(plugin.Name) +
            '" ' +
            checkedHtml +
            '><span></span></label>';
...
function getSimilarItemProvidersForTypeHtml(
    availableTypeOptions,
    libraryOptionsForType
) {
    let html = '';
    ...
    plugins = getOrderedPlugins(
        plugins,
        libraryOptionsForType.SimilarItemProviderOrder || []
    );
    if (!plugins.length) return html;

    html +=
        '<div class="similarItemProvider" data-type="' +
        availableTypeOptions.Type +
        '">';
    html +=
        '<h3 class="checkboxListLabel">' +
        globalize.translate(
            'HeaderTypeSimilarItemProviders',
            globalize.translate('TypeOptionPlural' + availableTypeOptions.Type)
        ) +
        '</h3>';
    html += '<div class="checkboxList paperList checkboxList-paperList">';
    ...
    plugins.forEach((plugin, index) => {
        html +=
            '<div class="listItem similarItemProviderItem sortableOption" data-pluginname="' +
            escapeHtml(plugin.Name) +
            '">';
        const isChecked = libraryOptionsForType.SimilarItemProviders
            ? libraryOptionsForType.SimilarItemProviders.includes(plugin.Name)
            : plugin.DefaultEnabled;
        const checkedHtml = isChecked ? ' checked="checked"' : '';
        html +=
            '<label class="listItemCheckboxContainer"><input type="checkbox" is="emby-checkbox" class="chkSimilarItemProvider" data-pluginname="' +
            escapeHtml(plugin.Name) +
            '" ' +
            checkedHtml +
            '><span></span></label>';
...
function renderImageFetchers(page, availableOptions, libraryOptions) {
    ...
    for (const availableTypeOptions of availableOptions.TypeOptions) {
        html += getImageFetchersForTypeHtml(
            availableTypeOptions,
            getTypeOptions(libraryOptions, availableTypeOptions.Type) || {}
        );
    }
...
function renderSimilarItemProviders(page, availableOptions, libraryOptions) {
    ...
    for (const availableTypeOptions of availableOptions.TypeOptions) {
        html += getSimilarItemProvidersForTypeHtml(
            availableTypeOptions,
            getTypeOptions(libraryOptions, availableTypeOptions.Type) || {}
        );
    }
...
function populateMetadataSettings(parent, contentType) {
    ...
    return ApiClient.getJSON(
        ApiClient.getUrl('Libraries/AvailableOptions', {
        ...
        .then((availableOptions) => {
            ...
            availableOptions.SubtitleFetchers.length
                ? parent
                      .querySelector('.subtitleDownloadSettings')
...
function showImageOptionsForType(type) {
    import('../imageOptionsEditor/imageOptionsEditor').then(
        ({ default: ImageOptionsEditor }) => {
            ...
            const availableOptions = getTypeOptions(
...
function onImageFetchersContainerClick(e) {
    const btnImageOptionsForType = dom.parentWithClass(
        e.target,
        'btnImageOptionsForType'
    );
    if (btnImageOptionsForType) {
        showImageOptionsForType(
            dom
                .parentWithClass(btnImageOptionsForType, 'imageFetcher')
...
function onSortableContainerClick(e) {
    ...
    if (btnSortable) {
        ...
        Array.prototype.forEach.call(
            list.querySelectorAll('.sortableOption'),
            adjustSortableListElement
        );
    }
}
...
function bindEvents(parent) {
    parent
        .querySelector('.metadataReaders')
        .addEventListener('click', onSortableContainerClick);
    parent
        .querySelector('.subtitleFetchers')
        .addEventListener('click', onSortableContainerClick);
    parent
        .querySelector('.metadataFetchers')
        .addEventListener('click', onSortableContainerClick);
    parent
        .querySelector('.lyricFetchers')
        .addEventListener('click', onSortableContainerClick);
    parent
        .querySelector('.mediaSegmentProviders')
        .addEventListener('click', onSortableContainerClick);
    parent
        .querySelector('.imageFetchers')
        .addEventListener('click', onImageFetchersContainerClick);
    parent
        .querySelector('.similarItemProviders')
        .addEventListener('click', onSortableContainerClick);

    parent
        .querySelector('#chkEnableEmbeddedTitles')
        .addEventListener('change', (e) => {
            parent
                .querySelector('.chkEnableEmbeddedExtrasTitlesContainer')
                .classList.toggle('hide', !e.currentTarget.checked);
        });
}
...
export function setContentType(parent, contentType) {
    if (contentType === 'homevideos' || contentType === 'photos') {
        parent
            .querySelector('.chkEnablePhotosContainer')
            .classList.remove('hide');
    } else {
        parent.querySelector('.chkEnablePhotosContainer').classList.add('hide');
    }

    const hasChapterOptions =
        !contentType /* Mixed */ || CHAPTER_CONTENT_TYPES.includes(contentType);
    parent
        .querySelector('.trickplaySettingsSection')
        .classList.toggle('hide', !hasChapterOptions);
    parent
        .querySelector('.chapterSettingsSection')
        .classList.toggle('hide', !hasChapterOptions);

    if (contentType === 'tvshows') {
        parent
            .querySelector('.chkAutomaticallyGroupSeriesContainer')
            .classList.remove('hide');
        parent
            .querySelector('.fldSeasonZeroDisplayName')
            .classList.remove('hide');
        parent
            .querySelector('#txtSeasonZeroName')
    ...
    if (
        ...
        parent
            .querySelector('.chkEnableEmbeddedTitlesContainer')
            .classList.add('hide');
        parent
            .querySelector('.chkEnableEmbeddedExtrasTitlesContainer')
            .classList.add('hide');
    ...
    parent
        .querySelector('.chkEnableLUFSScanContainer')
        .classList.toggle('hide', contentType !== 'music');

    if (contentType === 'tvshows') {
        parent
            .querySelector('.chkEnableEmbeddedEpisodeInfosContainer')
            .classList.remove('hide');
    } else {
        parent
            .querySelector('.chkEnableEmbeddedEpisodeInfosContainer')
    ...
    if (
        ...
        parent
            .querySelector('.fldAllowEmbeddedSubtitlesContainer')
            .classList.remove('hide');
    } else {
        parent
            .querySelector('.fldAllowEmbeddedSubtitlesContainer')
    ...
    if (contentType === 'music') {
        ...
        parent
            .querySelector('.audioTagSettingsSection')
            .classList.remove('hide');
    } else {
    ...
    parent
        .querySelector('.chkAutomaticallyAddToCollectionContainer')
        .classList.toggle(
            'hide',
            contentType !== 'movies' && contentType !== 'mixed'
        );
...
function setSubtitleFetchersIntoOptions(parent, options) {
    options.DisabledSubtitleFetchers = Array.prototype.map.call(
    ...
    options.SubtitleFetcherOrder = Array.prototype.map.call(
        parent.querySelectorAll('.subtitleFetcherItem'),
        (elem) => {
            return elem.getAttribute('data-pluginname');
        }
    );
...
function setLyricFetchersIntoOptions(parent, options) {
    options.DisabledLyricFetchers = Array.prototype.map.call(
        Array.prototype.filter.call(
            parent.querySelectorAll('.chkLyricFetcher'),
            (elem) => {
                return !elem.checked;
            }
        ),
    ...
    options.LyricFetcherOrder = Array.prototype.map.call(
        parent.querySelectorAll('.lyricFetcherItem'),
        (elem) => {
            return elem.getAttribute('data-pluginname');
        }
    );
...
function setMediaSegmentProvidersIntoOptions(parent, options) {
    options.DisabledMediaSegmentProviders = Array.prototype.map.call(
        Array.prototype.filter.call(
            parent.querySelectorAll('.chkMediaSegmentProvider'),
            (elem) => {
                return !elem.checked;
            }
        ),
    ...
    options.MediaSegmentProviderOrder = Array.prototype.map.call(
        parent.querySelectorAll('.mediaSegmentProviderItem'),
        (elem) => {
            return elem.getAttribute('data-pluginname');
        }
    );
...
function setMetadataFetchersIntoOptions(parent, options) {
    ...
    for (const section of sections) {
        ...
        typeOptions.MetadataFetchers = Array.prototype.map.call(
            Array.prototype.filter.call(
                section.querySelectorAll('.chkMetadataFetcher'),
                (elem) => {
                    return elem.checked;
                }
            ),
        ...
        typeOptions.MetadataFetcherOrder = Array.prototype.map.call(
            section.querySelectorAll('.metadataFetcherItem'),
            (elem) => {
                return elem.getAttribute('data-pluginname');
            }
        );
...
function setImageFetchersIntoOptions(parent, options) {
    ...
    for (const section of sections) {
        ...
        typeOptions.ImageFetchers = Array.prototype.map.call(
            Array.prototype.filter.call(
                section.querySelectorAll('.chkImageFetcher'),
                (elem) => {
                    return elem.checked;
                }
            ),
        ...
        typeOptions.ImageFetcherOrder = Array.prototype.map.call(
            section.querySelectorAll('.imageFetcherItem'),
            (elem) => {
                return elem.getAttribute('data-pluginname');
            }
        );
...
function setSimilarItemProvidersIntoOptions(parent, options) {
    ...
    for (const section of sections) {
        ...
        typeOptions.SimilarItemProviders = Array.prototype.map.call(
            Array.prototype.filter.call(
                section.querySelectorAll('.chkSimilarItemProvider'),
                (elem) => {
                    return elem.checked;
                }
            ),
        ...
        typeOptions.SimilarItemProviderOrder = Array.prototype.map.call(
            section.querySelectorAll('.similarItemProviderItem'),
            (elem) => {
                return elem.getAttribute('data-pluginname');
            }
        );
...
export function getLibraryOptions(parent) {
    const options = {
        ...
        EnableRealtimeMonitor: parent.querySelector('.chkEnableRealtimeMonitor')
            .checked,
        EnableLUFSScan: parent.querySelector('.chkEnableLUFSScan').checked,
        ExtractTrickplayImagesDuringLibraryScan: parent.querySelector(
            '.chkExtractTrickplayDuringLibraryScan'
        ).checked,
        SaveTrickplayWithMedia: parent.querySelector('.chkSaveTrickplayLocally')
            .checked,
        EnableTrickplayImageExtraction: parent.querySelector(
            '.chkExtractTrickplayImages'
        ).checked,
        ExtractChapterImagesDuringLibraryScan: parent.querySelector(
            '.chkExtractChaptersDuringLibraryScan'
        ).checked,
        EnableChapterImageExtraction: parent.querySelector(
            '.chkExtractChapterImages'
        ).checked,
        EnableInternetProviders: true,
        SaveLocalMetadata: parent.querySelector('#chkSaveLocal').checked,
        EnableAutomaticSeriesGrouping: parent.querySelector(
            '.chkAutomaticallyGroupSeries'
        ).checked,
        PreferredMetadataLanguage:
            parent.querySelector('#selectLanguage').value,
        ...
        AutomaticRefreshIntervalDays: parseInt(
            parent.querySelector('#selectAutoRefreshInterval').value,
            10
        ),
        EnableEmbeddedTitles: parent.querySelector('#chkEnableEmbeddedTitles')
            .checked,
        EnableEmbeddedExtrasTitles: parent.querySelector(
            '#chkEnableEmbeddedExtrasTitles'
        ).checked,
        EnableEmbeddedEpisodeInfos: parent.querySelector(
            '#chkEnableEmbeddedEpisodeInfos'
        ).checked,
        AllowEmbeddedSubtitles: parent.querySelector(
            '#selectAllowEmbeddedSubtitles'
        ).value,
        SkipSubtitlesIfEmbeddedSubtitlesPresent: parent.querySelector(
            '#chkSkipIfGraphicalSubsPresent'
        ).checked,
        SkipSubtitlesIfAudioTrackMatches: parent.querySelector(
            '#chkSkipIfAudioTrackPresent'
        ).checked,
        SaveSubtitlesWithMedia: parent.querySelector('#chkSaveSubtitlesLocally')
            .checked,
        SaveLyricsWithMedia: parent.querySelector('#chkSaveLyricsLocally')
            .checked,
        RequirePerfectSubtitleMatch: parent.querySelector(
            '#chkRequirePerfectMatch'
        ).checked,
        AutomaticallyAddToCollection: parent.querySelector(
            '#chkAutomaticallyAddToCollection'
        ).checked,
        PreferNonstandardArtistsTag: parent.querySelector(
            '#chkPreferNonstandardArtistsTag'
        ).checked,
        UseCustomTagDelimiters: parent.querySelector(
            '#chkUseCustomTagDelimiters'
        ).checked,
        MetadataSavers: Array.prototype.map.call(
            Array.prototype.filter.call(
                parent.querySelectorAll('.chkMetadataSaver'),
                (elem) => {
                    return elem.checked;
                }
            ),
    ...
    options.LocalMetadataReaderOrder = Array.prototype.map.call(
        parent.querySelectorAll('.localReaderOption'),
        (elem) => {
            return elem.getAttribute('data-pluginname');
        }
    );
    options.SubtitleDownloadLanguages = Array.prototype.map.call(
        Array.prototype.filter.call(
            parent.querySelectorAll('.chkSubtitleLanguage'),
            (elem) => {
                return elem.checked;
            }
        ),
    ...
    options.CustomTagDelimiters = parent
        .querySelector('#customTagDelimitersInput')
        .value.split('');
    options.DelimiterWhitelist = parent
        .querySelector('#tagDelimiterWhitelist')
        .value.split('\n')
...
export function setLibraryOptions(parent, options) {
    ...
    parent.querySelector('#selectCountry').value =
        options.MetadataCountryCode || '';
    parent.querySelector('#selectAutoRefreshInterval').value =
        options.AutomaticRefreshIntervalDays || '0';
    parent.querySelector('#txtSeasonZeroName').value =
        options.SeasonZeroDisplayName || 'Specials';
    parent.querySelector('.chkEnabled').checked = options.Enabled;
    parent.querySelector('.chkEnablePhotos').checked = options.EnablePhotos;
    parent.querySelector('.chkEnableRealtimeMonitor').checked =
        options.EnableRealtimeMonitor;
    parent.querySelector('.chkEnableLUFSScan').checked = options.EnableLUFSScan;
    parent.querySelector('.chkExtractTrickplayDuringLibraryScan').checked =
        options.ExtractTrickplayImagesDuringLibraryScan;
    parent.querySelector('.chkExtractTrickplayImages').checked =
        options.EnableTrickplayImageExtraction;
    parent.querySelector('.chkSaveTrickplayLocally').checked =
        options.SaveTrickplayWithMedia;
    parent.querySelector('.chkExtractChaptersDuringLibraryScan').checked =
        options.ExtractChapterImagesDuringLibraryScan;
    parent.querySelector('.chkExtractChapterImages').checked =
        options.EnableChapterImageExtraction;
    parent.querySelector('#chkSaveLocal').checked = options.SaveLocalMetadata;
    parent.querySelector('.chkAutomaticallyGroupSeries').checked =
        options.EnableAutomaticSeriesGrouping;
    parent.querySelector('#chkEnableEmbeddedTitles').checked =
        options.EnableEmbeddedTitles;
    parent
        .querySelector('.chkEnableEmbeddedExtrasTitlesContainer')
        .classList.toggle('hide', !options.EnableEmbeddedTitles);
    parent.querySelector('#chkEnableEmbeddedExtrasTitles').checked =
        options.EnableEmbeddedExtrasTitles;
    parent.querySelector('#chkEnableEmbeddedEpisodeInfos').checked =
        options.EnableEmbeddedEpisodeInfos;
    parent.querySelector('#selectAllowEmbeddedSubtitles').value =
        options.AllowEmbeddedSubtitles;
    parent.querySelector('#chkSkipIfGraphicalSubsPresent').checked =
        options.SkipSubtitlesIfEmbeddedSubtitlesPresent;
    parent.querySelector('#chkSaveSubtitlesLocally').checked =
        options.SaveSubtitlesWithMedia;
    parent.querySelector('#chkSaveLyricsLocally').checked =
        options.SaveLyricsWithMedia;
    parent.querySelector('#chkSkipIfAudioTrackPresent').checked =
        options.SkipSubtitlesIfAudioTrackMatches;
    parent.querySelector('#chkRequirePerfectMatch').checked =
        options.RequirePerfectSubtitleMatch;
    parent.querySelector('#chkAutomaticallyAddToCollection').checked =
        options.AutomaticallyAddToCollection;
    parent.querySelector('#chkPreferNonstandardArtistsTag').checked =
        options.PreferNonstandardArtistsTag;
    parent.querySelector('#chkUseCustomTagDelimiters').checked =
        options.UseCustomTagDelimiters;
    Array.prototype.forEach.call(
        parent.querySelectorAll('.chkMetadataSaver'),
        (elem) => {
            elem.checked = options.MetadataSavers
                ? options.MetadataSavers.includes(
                      elem.getAttribute('data-pluginname')
                  )
                : elem.getAttribute('data-defaultenabled') === 'true';
        }
    );
    Array.prototype.forEach.call(
        parent.querySelectorAll('.chkSubtitleLanguage'),
        (elem) => {
            elem.checked =
                ...
                options.SubtitleDownloadLanguages.includes(
                    elem.getAttribute('data-lang')
                );
        }
    );
    parent.querySelector('#customTagDelimitersInput').value =
        options.CustomTagDelimiters.join('');
    parent.querySelector('#tagDelimiterWhitelist').value =
        options.DelimiterWhitelist.filter((item) => item.trim()).join('\n');
    renderMetadataReaders(
        parent,
        getOrderedPlugins(
            parent.availableOptions.MetadataReaders,
