...
class MediaSegmentManager extends PlaybackSubscriber {
    ...
    private async fetchMediaSegments(
        ...
        try {
            const { data: mediaSegments } = await getMediaSegmentApi(
                api
            ).getItemSegments({ itemId, includeSegmentTypes });
            this.mediaSegments = mediaSegments.Items || [];
        } catch (err) {
            console.error(
                '[MediaSegmentManager] failed to fetch segments',
                err
            );
            this.mediaSegments = [];
        }
    ...
    skipSegment(mediaSegment: MediaSegmentDto) {
        ...
        if (
            mediaSegment.StartTicks !== undefined &&
            this.lastTime > mediaSegment.StartTicks
        ) {
            console.info(
            ...
            if (
                ...
                mediaSegment.EndTicks - mediaSegment.StartTicks <
                    TICKS_PER_SECOND
            ) {
                console.info(
            ...
            console.debug(
                '[MediaSegmentManager] skipping to next item in queue'
            );
    ...
    onPlayerPlaybackStart(_e: Event, state: PlayerState) {
        ...
        this.mediaSegmentTypeActions = Object.values(MediaSegmentType)
            ...
            .filter(
                ({ action }) => !!action && action !== MediaSegmentAction.None
            )
            .reduce(
                (acc, { type, action }) => {
        ...
        if (!Object.keys(this.mediaSegmentTypeActions).length) {
            console.info(
                '[MediaSegmentManager] user has no media segment actions enabled'
            );
            return;
        }
        ...
        if (!api) {
            console.error(
                '[MediaSegmentManager] no api instance available for server',
                serverId
            );
            return;
        }
        ...
        void this.fetchMediaSegments(
            api,
            itemId,
            Object.keys(this.mediaSegmentTypeActions).map(
                (t) => t as keyof typeof MediaSegmentType
            )
        );
    }
    ...
    onPlayerTimeUpdate() {
        if (this.hasSegments && this.mediaSegments.length) {
            const time =
                this.playbackManager.currentTime(this.player) *
