 ...
 */
const UpdatedAtCell = ({ cell }: { cell: MRT_Cell<PlaybackSessionListItem, unknown> }) => {
    const { dateFnsLocale } = useLocale();
    return format(cell.getValue<Date>(), 'Pp', { locale: dateFnsLocale });
};

const KindCell = ({ cell }: { cell: MRT_Cell<PlaybackSessionListItem, unknown> }) => (
    <>{globalize.translate(cell.getValue<string>())}</>
);

const MethodCell = ({ cell }: { cell: MRT_Cell<PlaybackSessionListItem, unknown> }) => (
    <PlaybackMethodChip method={cell.getValue<PlaybackMethod>()} />
);

const HasDiagnosticCell = ({ cell }: { cell: MRT_Cell<PlaybackSessionListItem, unknown> }) => {
    const hasDiagnostic = cell.getValue<boolean>();
    const label = globalize.translate(
        hasDiagnostic ? 'PlaybackDiagnosticAvailable' : 'PlaybackDiagnosticUnavailable'
    );
...
export const Component = () => {
    ...
    const columns = useMemo<MRT_ColumnDef<PlaybackSessionListItem>[]>(() => [
        ...
        },
        {
            ...
            header: globalize.translate('HeaderPlaybackDiagnosticAvailable'),
            size: 120,
    ...
    const mrTable = useMaterialReactTable<PlaybackSessionListItem>({
        ...
        ...(DEFAULT_TABLE_OPTIONS as Partial<MRT_TableOptions<PlaybackSessionListItem>>),
        mrtTheme,
