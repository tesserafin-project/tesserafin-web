...
import {
    encodePublishedServerUris,
    getPublishedServerUris,
    PublishedServerUris,
    splitString
} from 'apps/dashboard/features/networking/utils';
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const newConfig: NetworkConfiguration = {
        ...
        LocalNetworkAddresses: splitString(
            data.LocalNetworkAddresses?.toString()
        ),
        ...
        IsRemoteIPFilterBlacklist:
            data.IsRemoteIPFilterBlacklist?.toString() === 'blacklist'
    };
    ...
    if (data.PublishedServerUri) {
        ...
        publishedServerUri.internal =
            data.InternalPublishedServerUri?.toString();
        publishedServerUri.external =
            data.ExternalPublishedServerUri?.toString();
    }

    newConfig.PublishedServerUriBySubnet =
        encodePublishedServerUris(publishedServerUri);

    await getSystemApi(api).updateNamedConfiguration({
        key: CONFIG_KEY,
        body: newConfig
    });
...
export const Component = () => {
    const {
        ...
        isPending,
        isError
    } = useNamedConfiguration<NetworkConfiguration>(CONFIG_KEY);
    ...
    const [publishedUris, setPublishedUris] =
        useState<PublishedServerUris | null>();
    const [isUrisLoaded, setIsUrisLoaded] = useState(false);

    const onCertificatePathChange = useCallback(
    ...
    const onPublishedUriChange = useCallback(
    ...
    useEffect(() => {
        if (!isPending && !isError) {
            ...
            if (
                ...
                config.PublishedServerUriBySubnet.length > 0
            ) {
                const uris = getPublishedServerUris(
                    config.PublishedServerUriBySubnet
                );
                        ...
                        <Stack spacing={6}>
                            ...
                            <Stack spacing={3}>
                                ...
                                <TextField
                                    ...
                                    defaultValue={
                                        config.LocalNetworkAddresses?.join(
                                            ', '
                                        ) || ''
                                    }
                                />
                                ...
                                <TextField
                                    ...
                                    defaultValue={
                                        config.LocalNetworkSubnets?.join(
                                            ', '
                                        ) || ''
                                    }
                                />
                            ...
                            <Stack spacing={3}>
                                <Typography variant='h2'>
                                    {globalize.translate('HeaderHttpsSettings')}
                                </Typography>
                            ...
                            <Stack spacing={3}>
                                ...
                                >
                                    <MenuItem value={'whitelist'}>
                                        {globalize.translate('Whitelist')}
                                    </MenuItem>
                                    <MenuItem value={'blacklist'}>
                                        {globalize.translate('Blacklist')}
                                    </MenuItem>
                                </TextField>
                            ...
                            <Stack spacing={3}>
                                <Typography variant='h2'>
                                    {globalize.translate('HeaderNetworking')}
                                </Typography>
                            ...
                            <Stack spacing={3}>
                                <Typography variant='h2'>
                                    {globalize.translate('HeaderAutoDiscovery')}
                                </Typography>
                            ...
                            <Stack spacing={3}>
                                <Typography variant='h2'>
                                    {globalize.translate('HeaderPortRanges')}
                                </Typography>
                                ...
                                <FormControl>
                                    <FormControlLabel
                                        ...
                                        label={globalize.translate(
                                            'LabelUseSamePublishedServerUri'
                                        )}
                                    />
                                </FormControl>
                                ...
                                {useSamePublishedUri ? (
                                    ...
                                    <Stack direction='row' gap={1}>
                                        <TextField
                                            ...
                                            label={globalize.translate(
                                                'LabelInternalServerUri'
                                            )}
                                            helperText={globalize.translate(
                                                'LabelInternalServerUriHelp'
                                            )}
                                            defaultValue={
                                                publishedUris?.internal
                                            }
                                        />
                                        <TextField
                                            sx={{ flexGrow: 1 }}
                                            ...
                                            label={globalize.translate(
                                                'LabelExternalServerUri'
                                            )}
                                            helperText={globalize.translate(
                                                'LabelExternalServerUriHelp'
                                            )}
                                            defaultValue={
                                                publishedUris?.internal
                                            }
