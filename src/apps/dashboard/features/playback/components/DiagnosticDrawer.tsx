...
import type { MediaSourceSnapshot, PlaybackDiagnosticDetail, ReasonCode } from 'apps/dashboard/features/playback/api/types';
...
const DiagnosticDrawer = ({ sessionId, onClose }: DiagnosticDrawerProps) => {
    ...
    useEffect(() => {
        if (isNotFound) {
            onClose();
            setToastMessage(globalize.translate('PlaybackDiagnosticSessionNotFound'));
    ...
    const onCopyDiagnostic = useCallback(() => {
        ...
        copyDiagnosticToClipboard(detail).then(
            () => setToastMessage(globalize.translate('PlaybackDiagnosticCopied')),
            () => setToastMessage(globalize.translate('PlaybackDiagnosticCopyError'))
        );
    }, [ detail ]);
    ...
    const onExportFixture = useCallback(() => {
        ...
        exportFixture.mutate(sessionId, {
            onSuccess: blob => downloadBlob(blob, exportFixtureFilename(sessionId)),
            onError: mutationError => {
                ...
                const isDiagnosticGone = isAxiosError(mutationError) && mutationError.response?.status === 422;
                setToastMessage(globalize.translate(
                    isDiagnosticGone ? 'PlaybackDiagnosticUnavailable' : 'PlaybackDiagnosticFixtureExportError'
                ));
            }
        });
            ...
            <Drawer anchor='right' open={open} onClose={onClose}>
                ...
                >
                    <Stack direction='row' alignItems='center' justifyContent='space-between' sx={{ marginBottom: 2 }}>
                        <Typography id={titleId} variant='h1'>{globalize.translate('HeaderPlaybackDiagnostics')}</Typography>
                        {/*
                        ...
                        <Stack spacing={2}>
                            ...
                            {hasDiagnostic && !!detail.Reasoning && (
                                <Section title={globalize.translate('HeaderPlaybackReasoning')}>
                                    <ReasonTree root={detail.Reasoning} label={globalize.translate('HeaderPlaybackReasoning')} />
                                </Section>
                            )}
                            ...
                            <Stack direction='row' spacing={1}>
                                ...
                                >
                                    {globalize.translate('ButtonCopyDiagnostic')}
                                </Button>
                                <Button
                                    variant='outlined'
                                    startIcon={<DownloadIcon />}
                                    disabled={!hasDiagnostic || exportFixture.isPending}
                                    // Design doc §5.3: disabled whenever HasDiagnostic is false —
                                    // the server would otherwise answer with a 422 for a click
                                    // that can never succeed.
                                    title={hasDiagnostic ? undefined : globalize.translate('PlaybackDiagnosticUnavailable')}
