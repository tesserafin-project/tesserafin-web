...
const tree: ReasonNode = {
    ...
    Children: [
        { ...leaf('VideoRangeTypeNotSupported'), Children: [] },
        {
            ...
            Children: [
                { ...leaf('VideoCodecNotSupported'), Children: [] }
            ]
        }
    ]
};
...
describe('flattenVisibleReasonTree()', () => {
    it('returns every node in depth-first order when nothing is collapsed', () => {
        ...
        expect(result.map(item => item.path)).toEqual([ '0', '0.0', '0.1', '0.1.0' ]);
        expect(result.map(item => item.node.Code)).toEqual([
            'MethodChosen', 'VideoRangeTypeNotSupported', 'TonemapRequired', 'VideoCodecNotSupported'
        ]);
    });
    ...
    it('collapsing the root hides every descendant', () => {
        const result = flattenVisibleReasonTree(tree, path => path === ROOT_PATH);
...
describe('getReasonTreeKeyAction()', () => {
    ...
    const withOneCollapsed = flattenVisibleReasonTree(tree, path => path === '0.1');

    it('ArrowDown/ArrowUp move focus to the next/previous visible item', () => {
        expect(getReasonTreeKeyAction('ArrowDown', allExpanded, 0, () => false))
            .toEqual({ type: 'focus', path: '0.0' });
        expect(getReasonTreeKeyAction('ArrowUp', allExpanded, 2, () => false))
            .toEqual({ type: 'focus', path: '0.0' });
    });

    it('ArrowDown/ArrowUp do nothing past the ends of the list', () => {
        const lastIndex = allExpanded.length - 1;
        expect(getReasonTreeKeyAction('ArrowDown', allExpanded, lastIndex, () => false)).toBeUndefined();
        expect(getReasonTreeKeyAction('ArrowUp', allExpanded, 0, () => false)).toBeUndefined();
    });

    it('ArrowRight on a collapsed branch expands it without moving focus', () => {
        const collapsedIndex = withOneCollapsed.findIndex(item => item.path === '0.1');
        expect(getReasonTreeKeyAction('ArrowRight', withOneCollapsed, collapsedIndex, path => path === '0.1'))
            .toEqual({ type: 'toggle', path: '0.1', expand: true });
    });

    it('ArrowRight on an expanded branch moves focus to its first child', () => {
        expect(getReasonTreeKeyAction('ArrowRight', allExpanded, 0, () => false))
            .toEqual({ type: 'focus', path: '0.0' });
    });

    it('ArrowRight on a leaf does nothing', () => {
        const leafIndex = allExpanded.findIndex(item => item.path === '0.0');
        expect(getReasonTreeKeyAction('ArrowRight', allExpanded, leafIndex, () => false)).toBeUndefined();
    });

    it('ArrowLeft on an expanded branch collapses it without moving focus', () => {
        const branchIndex = allExpanded.findIndex(item => item.path === '0.1');
        expect(getReasonTreeKeyAction('ArrowLeft', allExpanded, branchIndex, () => false))
            .toEqual({ type: 'toggle', path: '0.1', expand: false });
    });

    it('ArrowLeft on a leaf or collapsed branch moves focus to its parent', () => {
        const leafIndex = allExpanded.findIndex(item => item.path === '0.1.0');
        expect(getReasonTreeKeyAction('ArrowLeft', allExpanded, leafIndex, () => false))
    ...
    it('ArrowLeft on an already-collapsed root does nothing (no parent to move to)', () => {
        ...
        expect(getReasonTreeKeyAction('ArrowLeft', rootCollapsed, 0, isCollapsed)).toBeUndefined();
    });

    it('Home/End jump to the first/last visible item', () => {
        expect(getReasonTreeKeyAction('Home', allExpanded, 2, () => false))
            .toEqual({ type: 'focus', path: '0' });
        expect(getReasonTreeKeyAction('End', allExpanded, 0, () => false))
            .toEqual({ type: 'focus', path: '0.1.0' });
    });

    it('returns undefined for an out-of-range current index', () => {
        expect(getReasonTreeKeyAction('ArrowDown', allExpanded, -1, () => false)).toBeUndefined();
    });
});
