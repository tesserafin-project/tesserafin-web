...
const Access = ({ userId }: AccessProps) => {
    ...
    const loadMediaFolders = useCallback(
        (user: UserDto, mediaFolders: BaseItemDto[]) => {
            const page = element.current;

            if (!page) {
                console.error(
                    '[userlibraryaccess] Unexpected null page reference'
                );
                return;
            }
            ...
            chkEnableAllFolders.checked = Boolean(
                user.Policy?.EnableAllFolders
            );
    ...
    const loadChannels = useCallback(
        (user: UserDto, channels: BaseItemDto[]) => {
            const page = element.current;
            ...
            for (const folder of channels) {
                const isChecked =
                    ...
                    user.Policy?.EnabledChannels?.indexOf(folder.Id || '') !=
                        -1;
                const checkedAttribute = isChecked ? ' checked="checked"' : '';
                itemsArr.push({
                    Id: folder.Id,
            ...
            chkEnableAllChannels.checked = Boolean(
                user.Policy?.EnableAllChannels
            );
    ...
    const loadDevices = useCallback(
        (user: UserDto, devices: DeviceInfoDto[]) => {
            const page = element.current;
            ...
            chkEnableAllDevices.checked = Boolean(
                user.Policy?.EnableAllDevices
            );
    ...
    const loadData = useCallback(() => {
        ...
        const promise3 = window.ApiClient.getJSON(
            window.ApiClient.getUrl('Channels')
        );
        const promise4 = window.ApiClient.getJSON(
            window.ApiClient.getUrl('Devices')
        );
    ...
    useEffect(() => {
        ...
        const onSubmit = (e: Event) => {
            ...
            window.ApiClient.getUser(userId)
                ...
                .catch((err) => {
                    console.error(
                        '[userlibraryaccess] failed to fetch user',
                        err
                    );
                });
            e.preventDefault();
        ...
        const saveUser = (user: UserDto) => {
            ...
            user.Policy.EnabledFolders = user.Policy.EnableAllFolders
                ...
                : Array.prototype.filter
                      .call(page.querySelectorAll('.chkFolder'), function (c) {
                          return c.checked;
                      })
                      .map(function (c) {
            ...
            user.Policy.EnabledChannels = user.Policy.EnableAllChannels
                ? []
                : Array.prototype.filter
                      .call(page.querySelectorAll('.chkChannel'), function (c) {
                          return c.checked;
                      })
                      .map(function (c) {
                          return c.getAttribute('data-id');
            ...
            user.Policy.EnabledDevices = user.Policy.EnableAllDevices
                ? []
                : Array.prototype.filter
                      .call(page.querySelectorAll('.chkDevice'), function (c) {
                          return c.checked;
                      })
                      .map(function (c) {
                          return c.getAttribute('data-id');
            ...
            window.ApiClient.updateUserPolicy(user.Id, user.Policy)
                ...
                .catch((err) => {
                    console.error(
                        '[userlibraryaccess] failed to update user policy',
                        err
                    );
                });
        };
        ...
        (
            page.querySelector('.userLibraryAccessForm') as HTMLFormElement
