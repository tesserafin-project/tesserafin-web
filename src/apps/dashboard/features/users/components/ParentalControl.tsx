...
function handleSaveUser(
    ...
    return (user: UserDto) => {
        ...
        const parentalRatingIndex = parseInt(
            (
                page.querySelector(
        ...
        const parentalRating = parentalRatingsRef.current[
            parentalRatingIndex
        ] as ParentalRating;
        ...
        userPolicy.MaxParentalSubRating = Number.isNaN(subScore)
            ? null
            : subScore;
        ...
        ServerConnections.getCurrentApiClientAsync()
            ...
            .catch((err) => {
                console.error(
                    '[userparentalcontrol] failed to update user policy',
                    err
                );
            });
    };
...
const ParentalControl = ({ userId }: ParentalControlProps) => {
    ...
    const loadUnratedItems = useCallback((user: UserDto) => {
        ...
        if (!page) {
            console.error(
                '[userparentalcontrol] Unexpected null page reference'
            );
            return;
        }
        ...
        for (const item of items) {
            const isChecked =
                user.Policy?.BlockUnratedItems?.indexOf(item.value) != -1;
        ...
        const blockUnratedItems = page.querySelector(
            '.blockUnratedItems'
        ) as HTMLDivElement;
    ...
    const loadUser = useCallback(
        (user: UserDto, allParentalRatings: ParentalRating[]) => {
            ...
            for (
                let i = 0, length = allParentalRatings.length;
                i < length;
                i++
            ) {
                ...
                if (ratings.length) {
                    ...
                    if (
                        lastRating.RatingScore?.score ===
                            rating.RatingScore?.score &&
                        lastRating.RatingScore?.subScore ==
                            rating.RatingScore?.subScore
                    ) {
    ...
    useEffect(() => {
        ...
        const getSchedulesFromPage = () => {
            return Array.prototype.map.call(
                page.querySelectorAll('.liSchedule'),
                function (elem) {
                    return {
        ...
        const getAllowedTagsFromPage = () => {
            return Array.prototype.map.call(
                page.querySelectorAll('.allowedTag'),
                function (elem) {
        ...
        const getBlockedTagsFromPage = () => {
            return Array.prototype.map.call(
                page.querySelectorAll('.blockedTag'),
                function (elem) {
                    return elem.getAttribute('data-tag');
                }
            ) as string[];
        ...
        const onSubmit = (e: Event) => {
            ...
            window.ApiClient.getUser(userId)
                ...
                .catch((err) => {
                    console.error(
                        '[userparentalcontrol] failed to fetch user',
                        err
                    );
                });
            e.preventDefault();
        ...
        (
            page.querySelector('#btnAddSchedule') as HTMLButtonElement
        ).addEventListener('click', accessSchedulesPopupCallback);
        (
            page.querySelector('#btnAddAllowedTag') as HTMLButtonElement
        ).addEventListener('click', showAllowedTagPopup);
        (
            page.querySelector('#btnAddBlockedTag') as HTMLButtonElement
        ).addEventListener('click', showBlockedTagPopup);
        (
            page.querySelector('.userParentalControlForm') as HTMLFormElement
        ).addEventListener('submit', onSubmit);

        return () => {
            (
                page.querySelector('#btnAddSchedule') as HTMLButtonElement
            ).removeEventListener('click', accessSchedulesPopupCallback);
            (
                page.querySelector('#btnAddAllowedTag') as HTMLButtonElement
            ).removeEventListener('click', showAllowedTagPopup);
            (
                page.querySelector('#btnAddBlockedTag') as HTMLButtonElement
            ).removeEventListener('click', showBlockedTagPopup);
            (
                page.querySelector(
    ...
    useEffect(() => {
        ...
        (
            page.querySelector('#selectMaxParentalRating') as HTMLSelectElement
        ).value = String(maxParentalRating);
