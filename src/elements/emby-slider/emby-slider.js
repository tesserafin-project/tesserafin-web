...
let supportsValueAutoSnap = false;
{
    ...
    if (!supportsValueAutoSnap) {
        console.debug(
            "[EmbySlider] HTMLInputElement doesn't snap value - use workaround."
        );
    }
}
    ...
    if (range.getMarkerInfo) {
        ...
        if (!range.markerInfo || !isEqual(range.markerInfo, newMarkerInfo)) {
            ...
            range.markerInfo.forEach(() => {
                markersHtml +=
                    '<span class="sliderMarker" aria-hidden="true"></span>';
            });
            range.markerContainerElement.innerHTML = markersHtml;

            range.markerElements =
                range.markerContainerElement.querySelectorAll('.sliderMarker');
        }
    }
...
EmbySliderPrototype.attachedCallback = function () {
    ...
    htmlToInsert +=
        '<div class="sliderBubbleTrack"><div class="sliderBubble hide"></div></div>';

    containerElement.insertAdjacentHTML('beforeend', htmlToInsert);

    this.sliderBubbleTrack =
        containerElement.querySelector('.sliderBubbleTrack');
    this.backgroundLower = containerElement.querySelector(
        '.mdl-slider-background-lower'
    );
    this.backgroundUpper = containerElement.querySelector(
    ...
    this.markerContainerElement = containerElement.querySelector(
    ...
    if (browser.iOS) {
        dom.addEventListener(
            ...
            function (e) {
                ...
                const fraction = mapClientToFraction(
                    this,
...
function onKeyDown(e) {
    switch (keyboardnavigation.getKeyName(e)) {
        ...
        case 'Left':
            stepKeyboard(
                this,
                -normalizeSliderStep(this, this.keyboardStepDown)
            );
            e.preventDefault();
            e.stopPropagation();
            break;
...
EmbySliderPrototype.setIsClear = function (isClear) {
    ...
    if (backgroundLower) {
        if (isClear) {
            ...
            backgroundLower.classList.remove(
