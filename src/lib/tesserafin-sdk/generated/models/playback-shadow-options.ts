...
export interface PlaybackShadowOptions {
    ...
    /**
     * Gets or sets the explicit role the v2 engine plays for live playback (PR115a). Defaults to Tesserafin.Model.Configuration.PlaybackEngineMode.Legacy. When left at that default, a pre-PR115a configuration with Tesserafin.Model.Configuration.PlaybackShadowOptions.Enabled set still gets shadow behavior - see M:Tesserafin.Model.Configuration.PlaybackShadowOptions.GetEffectiveMode.
     * @type {PlaybackEngineMode}
     * @memberof PlaybackShadowOptions
     */
    'Mode'?: PlaybackEngineMode;
    /**
     * Gets or sets the fraction of eligible playback decisions (0.0-1.0) that actually run the shadow comparison when Tesserafin.Model.Configuration.PlaybackShadowOptions.Enabled is true. 1.0 (the default) means every decision is shadowed; lower values sample. PR104: clamped to [0, 1] on set - a value outside that range (including NaN, clamped to 0) has no meaningful sampling interpretation, so it is corrected rather than left to produce nonsensical behavior downstream (for example a negative rate always sampling out, or a rate above 1 being indistinguishable from 1).
     * @type {number}
     * @memberof PlaybackShadowOptions
     */
    'SampleRate'?: number;
    /**
     * Gets or sets the PR115d operational stop-threshold guard configuration - see Tesserafin.Model.Configuration.PlaybackStopThresholdOptions for why this is a sibling type rather than fields on this class directly. Never null; defaults to a fresh, enabled instance with its own PR115d defaults, so a configuration that predates PR115d still gets the guard\'s protection without needing to add anything.
