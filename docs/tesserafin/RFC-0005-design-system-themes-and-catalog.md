...
- **Relation** : RFC-0003 (`docs/tesserafin/RFC-0003-product-rupture.md`) §9.5 (mandat design system —
  ...
  §12 (frontière plugins/Plugin SDK v2) ; RFC-0001 (`docs/tesserafin/RFC-0001-vision-and-feasibility.md`)
  pilier 1 (« Interface entièrement modernisée ») ; `design-tesserafin-shell-and-routing.md` §3.3 (états
  loading/error/empty) et §4 (checklist de migration route par route) ; RFC-0004
  (`docs/tesserafin/RFC-0004-platform-scope-and-legacy-tv-removal.md`) §4 (UX télévision générique —
...
1. **Le système de thèmes hérité** (`src/themes/`) : six thèmes, deux couches faiblement couplées
   ...
   index.ts`, les dossiers `src/themes/<id>/`). Détaillé en §4.1.
2. **L'absence de kit de composants** : `src/ui/` n'existe pas. La route `/home` réécrite consomme
   ...
   pont de types (`itemDtoAdapter.ts`) vers l'ancien `cardBuilder`. Détaillé en §4.2.

Sans ce RFC, chaque route migrée (`/library/:libraryId` en W13.7, `/title/:itemId` en W13.9, cf. §11)
...
s'appuie sur un socle commun : un principe cross-platform qui borne ce qui s'unifie de ce qui reste
natif (§3), quatre concepts distincts (§5), un design system `src/ui/` (§6), un contrat de thème
formel (§7), trois thèmes intégrés (§8), une stratégie de chargement/budget/persistance (§9), et un
catalogue communautaire (§10).

Ce RFC porte aussi, dès sa conception, une contrainte produit que RFC-0003/RFC-0001 anticipent sans
la trancher : Reefin Web est le premier client d'une plateforme appelée à s'étendre à des
applications natives Android/Android TV (Jetpack Compose) et iOS/tvOS (SwiftUI). Le contrat de
thème et le design system spécifiés ici ne doivent pas être conçus comme des artefacts Web-only
qu'il faudrait réécrire au démarrage de ces apps : le principe qui borne ce qui s'unifie entre
plateformes et ce qui reste natif, ainsi que le socle qui le porte, sont détaillés en §3.
...
- Le principe directeur d'unification cross-platform du langage visuel et du contrat de thème avec
  les futures apps natives Android/iOS/TV, et le socle `tesserafin-design` qui le porte (§3).
- La séparation conceptuelle design system / thème / préréglage de disposition / plugin (§5).
- L'architecture du futur `src/ui/` : liste de composants, contrat de variantes sémantiques, slots
  publics (§6).
- Le contrat de thème (`ThemeDefinition`, profils d'interaction, format de paquet communautaire v1)
  et la migration du préfixe `jf` vers `--rf-*` (§7).
...
- La stratégie de chargement par chunk, les budgets, et la persistance de la sélection utilisateur —
  locale par appareil, en anticipant un choix de compte synchronisable (§9).
- Le modèle du catalogue officiel de thèmes communautaires — pipeline de validation, tiers de
  compatibilité, sans backend de téléversement en v1 (§10).
- Le séquencement en tranches W13.5 → W14.0+ (§11).
...
- L'implémentation elle-même : ce RFC ne produit aucun code. La tranche W13.5 est ce document ; le
  premier code arrive en W13.6 (§11).
- Le Plugin SDK v2 (RFC-0003 §12) : les thèmes et les plugins partageront à terme la même galerie
  (§10.6) mais restent deux pipelines de sécurité distincts ; ce RFC ne spécifie pas le sandbox plugin.
- La refonte du lecteur (machine à états, RFC-0003 §9.3) et la configuration guidée matérielle
  (RFC-0003 §9.4) : hors sujet design system, sauf pour les composants `PlayerControls` listés en §6
  qui n'en spécifient que l'habillage, pas la logique.
- La synchronisation serveur du choix de thème (`DisplayPreferences`) : la décision v1 reste
  localStorage-only (§9.3), mais le contrat de thème et le manifeste anticipent dès maintenant une
  préférence par appareil doublée d'un suivi de compte — la bascule effective vers une
  synchronisation serveur reste une question ouverte, reformulée en ce sens (§12.1).

---

## 3. Principe cross-platform : unifier le langage, pas l'interface

Reefin Web n'est que le premier client d'une plateforme appelée à s'étendre à des applications
natives : Android/Android TV en Jetpack Compose, iOS/tvOS en SwiftUI. Ce RFC arbitre, dès la
conception du contrat de thème et non après coup, ce qui doit être unifié entre ces clients et ce
qui doit rester natif à chaque plateforme — jamais l'interface elle-même. Composants React, Compose
et SwiftUI restent des implémentations natives indépendantes ; ce qui s'unifie, c'est le langage
visuel qu'elles expriment.

### 3.1 À unifier / à garder natif

| À unifier | À garder natif |
| --- | --- |
| Couleurs sémantiques | Composants React, Compose et SwiftUI |
| Typographie et échelle | Navigation propre à chaque OS |
| Espacements et rayons | Gestes tactiles |
| Ombres, flou, mouvement | Focus et télécommande TV |
| Icônes et assets | Menus et conventions iOS/Android |
| Noms/états des composants | Contraintes de performance |
| Manifestes de thèmes | Disposition exacte des écrans |

Les composants gardent les mêmes **concepts** partout — `MediaCard`, `MediaShelf`,
`HeroSpotlight`, `ProgressIndicator`, `NavigationRail` — mais chaque plateforme en fournit sa
propre implémentation native ; c'est la colonne de gauche, pas la colonne de droite, que le contrat
de thème (§7) et le socle du §3.2 rendent portable.

### 3.2 Socle `tesserafin-design` (nom provisoire)

Un socle indépendant, distinct de `src/ui/` (§6), contient uniquement ce qui doit être partagé :
schéma des thèmes, design tokens, icônes/assets communs, les trois thèmes officiels (§8), et les
règles de compatibilité/validation (§10). Une génération produit les formats par plateforme :

```mermaid
flowchart TD
    A["Thème déclaratif"] --> B["Tokens Web"]
    A --> C["Tokens Android"]
    A --> D["Tokens Apple"]
    B --> E["React / MUI"]
    C --> F["Jetpack Compose"]
...
Sorties attendues : `tokens.css` + `theme.ts` pour Reefin Web, `ReefinTheme.kt` pour Android/
Android TV, `ReefinTheme.swift` pour iOS/tvOS. Le renderer Web est le premier implémenté (§11,
W13.6) ; les renderers Compose et SwiftUI n'arrivent qu'au démarrage des apps correspondantes — ce
séquencement évite d'enfermer le format de thème dans des hypothèses MUI/CSS sans ralentir Reefin
Web ni engager de travail natif prématuré. Le schéma des thèmes de `tesserafin-design` est le format
universel dont l'interface `ThemeDefinition` (§7.1) est la spécification TypeScript consommée par
Reefin Web ; les autres plateformes consommeront des sérialisations générées du même schéma, jamais
une redéfinition parallèle.

---

## 4. État des lieux — audit

### 4.1 Système de thèmes hérité (`src/themes/`)
...
motive la décision §8.1 de n'en faire ni un des trois thèmes structurels, ni un préréglage trivial.
...
- CSS legacy **déjà différé** : `src/components/ThemeCss.tsx:10,23` injecte
  ...
  thème actif est téléchargé — pattern à conserver tel quel (§9.1).
...
- Catalogue des thèmes proposés : `src/config.json:4-31` (liste `{name,id,color,default}`) via
  ...
  `colorSchemes`, sans synchronisation garantie (§7.4).
...
thème côté JS, alors que le CSS l'est déjà (§9.1). Budget actuel :
...
### 4.2 Surface `/home` et shell modern
...
- L1 : `import { BaseItemKind } from '@jellyfin/sdk/lib/generated-client/models/base-item-kind'` —
  import direct `@jellyfin/sdk` alors que le reste de `features/home/*` consomme `lib/tesserafin-sdk`.
  Incohérence à corriger en W13.6 (§11).
...
slice (états loading/error/empty/success conformes à `design-tesserafin-shell-and-routing.md` §3.3), mais
...
doivent reprendre (§6). `features/home/utils/itemDtoAdapter.ts:4-18` illustre le coût de l'absence de
contrat commun : un cast `item as unknown as ItemDto` sert de pont entre `BaseItemDto` (tesserafin-sdk) et
...
sans pont SCSS↔MUI (question ouverte §12.6).
...
## 5. Séparation de quatre concepts
...
| Concept | Rôle |
| --- | --- |
| **Design system** | Composants et comportements stables fournis par Reefin (`src/ui/`, §6) — structure, accessibilité, API. |
| **Thème** | Couleurs, typographie, rayons, surfaces, ombres, flou, mouvement — un jeu de valeurs de tokens (§7). |
...
c'est précisément ce que le format de paquet déclaratif du §7.3 interdit, et ce que le préréglage de
disposition (composition de `HeroSpotlight`, densité, choix de shelf) doit porter à sa place. Cette
distinction est ce qui permet au §8.2 de réimplémenter « l'esprit Abyss » comme thème Glass sans
...
`src/ui/` est introduit progressivement à partir de W13.6 (§11), candidat naturel à devenir à terme un
...
manque aujourd'hui à `HomeSection.tsx` (§4.2), qui expose des classes historiques directement. Les
points de personnalisation publics passent par des attributs `data-rf-slot`, seule surface que le CSS
d'un thème communautaire est autorisé à cibler (§7.3) ; cibler autre chose (une classe MUI générée,
...
le préfixe `jf` hérité (§4.1 : 107 occurrences, 5 fichiers, `cssVarPrefix` défini
`src/themes/index.ts:14`). Le remplacement se fait fichier par fichier au fil de la migration de
chaque route (§11, W13.6), pas en un renommage global — le risque documenté est qu'un oubli de
`var(--jf-x, $fallback)` ne casse aucun build (pas d'erreur TypeScript/webpack) mais produit une
couleur silencieusement incorrecte (§4.1, cf. aussi risque 7 de l'audit thèmes).
...
styles `:focus-visible` des composants `emby-*`) et `design-tesserafin-shell-and-routing.md` §3.4 exige
...
respectent `prefers-reduced-motion` par défaut — repris en détail pour les thèmes en §8.2 et §8.3, et
formalisé côté contrat par le profil `reducedMotion` (§7.2).

---

## 7. Contrat de thème

### 7.1 Interface `ThemeDefinition`
...
`data-rf-slot` exposés par `src/ui/` (§6). Cette interface est ce qui remplace, côté runtime, les deux
couches aujourd'hui faiblement couplées de `src/themes/` (palette MUI d'un côté, CSS Sass legacy de
l'autre, §4.1) par une source unique consommée à la fois par le rendu MUI et par le CSS exporté.
Le détail des profils d'interaction qui paramètrent ces tokens par plateforme et par capacité
d'appareil est spécifié en §7.2.

### 7.2 Profils d'interaction

Le contrat ne distingue pas seulement des **plateformes** (`web`, `android`, `ios`, `tv`) mais
surtout des **profils**, orthogonaux aux plateformes et applicables à `ThemeDefinition` :

| Axe | Valeurs |
| --- | --- |
| Entrée | `pointer` (souris/clavier), `touch` (smartphone/tablette), `remote` (TV dix pieds) |
| Espace disponible | `compact`, `medium`, `expanded` |
| Accessibilité | `reducedMotion`, `reducedTransparency` |
| Matériel | `lowPower` (TV/boîtiers modestes) |

Un thème reste reconnaissable sans imposer le même rendu technique partout : Glass (§8.2) utilise
le flou natif sur desktop et smartphone puissant, mais bascule sur une surface translucide ou
opaque en profil `remote`+`lowPower` — c'est le même mécanisme que le fallback opaque déjà exigé
pour Glass (§8.2) et que le respect de `prefers-reduced-motion` déjà porté par `FocusRing` (§6),
formalisé ici comme propriété du contrat plutôt que comme cas particulier par thème.

### 7.3 Format de paquet communautaire v1 — strictement déclaratif
...
plus jamais pouvoir reproduire, quel que soit son niveau de licence produit dans le catalogue (§10).

Le manifeste (`theme.json`) prévoit dès maintenant la compatibilité multi-plateformes et des
overrides par profil (§7.2), même si seul le renderer Web les consomme en v1 (§3.2) :
...
```json
{
  ...
  "profiles": {
    "remote": { "blur": "reduced", "density": "comfortable" },
    "touch": { "density": "compact" }
  }
}
```

Le champ `compatibility` détermine le tier attribué au thème dans le catalogue (§10.2) ; le champ
`profiles` porte les overrides décrits en §7.2, absent ou vide pour un thème qui ne cible qu'un seul
profil.

### 7.4 Trois sources de vérité à converger

L'audit (§4.1) identifie trois sources de vérité aujourd'hui indépendantes pour le catalogue de
...
registre unique de thèmes**, construit à partir des `ThemeDefinition` (§7.1) — deux des trois sources
...
runtime) est un choix d'implémentation laissé à W13.6 (§11), pas figé ici.

### 7.5 Abyss — inspiration assumée, pas dépendance

Le thème communautaire Abyss (`https://github.com/AumGupta/abyss-jellyfin`, licence MIT) est la
référence visuelle explicite du thème Reefin Glass (§8.2), et son analyse illustre concrètement
pourquoi le format déclaratif du §7.3 existe : `abyss.css` — environ 2 315 lignes, 261 déclarations
...
de ces fichiers n'est présent dans `reefin-web`. Le principe retenu (§8.2) : reprendre le **langage
...
conforme au format §7.3 alors qu'une reprise directe du CSS original ne le serait pas.

---

## 8. Trois thèmes intégrés
...
### 8.1 Reefin Classic (thème par défaut)
...
directement pour `blueradiance` et `wmc` (pures variations de palette, §4.1), et par réduction
assumée pour `appletv`, dont les divergences structurelles (dégradés, `border-radius` custom, §4.1)
...
hardcodées et son absence de `@use base` en font une dette isolée (§4.1, §12.2) plutôt qu'un simple
préréglage de couleur — sa réécriture ou son abandon est une question ouverte (§12.2), pas une
décision prise par ce RFC.

**Cross-platform (§3)** : Classic est la référence universelle du contrat — c'est le thème qui doit
fonctionner sans dégradation sur les quatre plateformes (web, android, ios, tv) et sert de fallback
si un thème tiers ne couvre pas un profil donné (§7.2).

### 8.2 Reefin Glass

Réimplémentation native de l'esprit du thème communautaire Abyss (§7.5) : verre dépoli
...
dégrader silencieusement en surface transparente illisible. Livré en W13.8 (§11), avec captures
Playwright dédiées desktop/mobile/TV.

**Cross-platform (§3)** : le langage visuel (verre dépoli, surfaces superposées) est universel, mais
son intensité varie par profil (§7.2) — flou complet sur desktop/smartphone puissant, surface
translucide ou opaque sur TV `lowPower`, sans jamais perdre l'identité Glass.
...
Livré en W13.9 (§11), en même temps que `HeroSpotlight` et les pastilles de métadonnées dont il est le
terrain d'expression naturel.

**Cross-platform (§3)** : Cinema trouve son terrain naturel sur TV (grands backdrops, chrome
minimal) ; décliné sur mobile avec une composition plus compacte et moins de backdrops simultanés,
pour rester lisible et performant sur les profils `touch`/`compact` (§7.2).

---

## 9. Chargement, budget et persistance

### 9.1 Chargement par chunk
...
réinventé (§4.1). Ce qui doit changer : les 6 `colorSchemes` MUI, aujourd'hui importés de façon
...
critique, chargés séparément si le thème les utilise (§4.1, risque 9 de l'audit). Ce chiffre est une
...
(`package.json:148`) — c'est la gate à exécuter avant toute PR de ce RFC qui touche au bundle (§11).
...
synchronisation serveur (`DisplayPreferences`) dans le périmètre W13.5-W14.0. Ce choix v1 ne préjuge
pas de la forme finale : le contrat de thème et le manifeste (§7.3) anticipent dès maintenant un
modèle à deux niveaux — un **choix de compte** (thème préféré, synchronisable entre appareils) et un
**override local par appareil**, prioritaire tant qu'il est défini (ex. Glass sur l'ordinateur du
salon, Cinema sur la TV du même compte). Rien dans le format de thème (§7) ne suppose aujourd'hui un
unique thème par utilisateur — c'est une contrainte volontairement non prise dans ce RFC, pas une
impossibilité technique découverte plus tard. La synchronisation serveur effective reste une question
ouverte (§12.1), pas une régression : le comportement actuel (aucun partage entre appareils) est
simplement maintenu, pas dégradé. Ce qui change : le catalogue de thèmes disponibles pour la
sélection provient du registre unique (§7.4) plutôt que de `src/config.json` isolément.

---

## 10. Catalogue officiel (marketplace v1 sans backend)
...
### 10.2 Tiers de thèmes

Le manifeste (§7.3) permet de distinguer trois tiers dans le catalogue :

| Tier | Définition |
| --- | --- |
| **Universal Theme** | Validé sur Web, Android, iOS et TV (`compatibility` couvre les quatre plateformes, §7.3) |
| **Platform Theme** | Compatible avec seulement certaines plateformes — badge affiché en galerie |
| **Official Theme** | Fourni et maintenu par le mainteneur (Classic/Glass/Cinema, §8) |

Les thèmes communautaires restent strictement déclaratifs (§7.3), sans code exécutable — un paquet
conforme est installable sur les apps officielles sans réécriture manuelle, quel que soit son tier.
Temporairement, les thèmes communautaires **Web-only sont autorisés** : ils apparaissent comme
Platform Theme avec un badge « web » explicite, en attendant que les renderers Compose/SwiftUI
existent (§3.2) pour les valider sur les autres plateformes.

### 10.3 Validation automatique

| Critère | Détail |
| --- | --- |
| Schéma + identifiant unique | `theme.json`/`tokens.json` conformes au schéma publié (`theme.schema.json`, §11 W14.0), id de thème non collisionnant dans le registre |
| Licence | SPDX obligatoire dans `theme.json` |
| Compatibilité | Plage de versions Reefin explicite par plateforme (`compatibility`, §7.3), tier attribué automatiquement selon couverture (§10.2) |
| Sécurité | Aucun code exécutable, aucune ressource distante (§7.3) |
| Poids | Respect du budget par thème (§9.2) |
...
| Mouvement/transparence | Respect de `prefers-reduced-motion` et du mode sans transparence (fallback opaque, §8.2) |
| Couverture | Rendu vérifié sur une page de référence contenant tous les composants `src/ui/` (« Theme Lab », §11 W14.0) |
| Intégrité | Checksum + signature de l'artefact publié |

### 10.4 Approbation manuelle

La validation automatique (§10.3) est une condition nécessaire, pas suffisante : un mainteneur humain
...
### 10.5 URL du catalogue — configurable, jamais figée en dur

Le nom de domaine définitif du site catalogue n'est pas tranché (conflit de nom/marque non résolu,
question ouverte §12.5). **L'URL du registre/catalogue doit être configurable** dans Reefin, jamais
codée en dur comme `reefin.com` dans le format de thème ou dans le client — le format de paquet
(§7.3) et le protocole de résolution de registre ne doivent dépendre d'aucun domaine particulier. Une
...
### 10.6 Cohabitation future avec les plugins
...
thèmes sont déclaratifs et validés statiquement (§10.3), les plugins sont exécutables et nécessitent un
...
## 11. Roadmap en tranches

### W13.5 — ce RFC

Rédaction et acceptation de ce document. **Aucun code produit avant acceptation.** Ce document fixe
dès cette tranche les décisions cross-platform suivantes, à respecter par les tranches suivantes :

1. Le contrat de thèmes (§7) est cross-platform dès sa conception, pas amendé après coup.
2. Le renderer Web (`tokens.css`/`theme.ts`) est implémenté en premier (§3.2).
3. Un jeu de **fixtures de référence** sera produit pour Android/iOS/TV — les thèmes officiels
   sérialisés dans le format universel, consommables comme golden files par les futurs renderers.
4. Les renderers Compose/SwiftUI n'arrivent qu'au démarrage des apps correspondantes (§3.2).
5. Les trois thèmes officiels (§8) respectent le contrat universel dès leur livraison.
6. Les thèmes communautaires Web-only sont autorisés temporairement, signalés en galerie (§10.2).
...
- `src/ui/` créé avec les premiers composants de §6 nécessaires à `/home` (`MediaShelf`, `MediaCard`,
  `Tabs`, `LoadingState`/`EmptyState`/`ErrorState`).
- Tokens `--rf-*` introduits, remplacement progressif du préfixe `jf` démarré (§4.1, §6).
- Registre unique de thèmes (§7.4) et chargement asynchrone par chunk pour les `colorSchemes` MUI
  (§9.1).
- Génération des tokens Web (`tokens.css`/`theme.ts`) depuis le contrat universel (§7), pas de format
  Web-privé — Reefin Web reste le premier renderer implémenté (§3.2).
- Première passe de fixtures de référence : Classic sérialisé dans le format universel `tesserafin-design`
  (§3.2, W13.5 point 3), même si aucun renderer natif ne les consomme encore.
...
- Refonte de `/home` sur `MediaShelf`/`MediaCard`/`Tabs` et les états standards, en remplacement de
  `HomeSection.tsx` et des classes historiques (§4.2).
- Retrait de l'import résiduel `@jellyfin/sdk` de `home.tsx:1` (§4.2) au profit de `lib/tesserafin-sdk`.
- Livraison de Reefin Classic (§8.1).
- Critères de vérification :
  - `npm run validate:full` vert (typecheck, lint, stylelint, tests, build, `verify:bundle-budget`
    local — §9.2).
  - Test Vitest sur au moins un composant `src/ui/` (premier test de composant du dépôt, cf. §4.2 —
  ...
  - Re-grep `jf` dans `src/themes/` : progression mesurée par rapport à la baseline 107 occurrences/5
    fichiers (§4.1).
  - Classic vérifié conforme au contrat universel (schéma + profils, §7).
...
- Chunk séparé pour Glass, conforme au chargement par thème (§9.1).
- Critères de vérification :
...
- Livraison de Reefin Cinema (§8.3).
- Tests des trois thèmes (Classic/Glass/Cinema) desktop/mobile/TV.
- Critères de vérification :
  - Parcours Playwright `/title/:itemId` pour les trois thèmes.
  - `npm run validate:full` vert.
  - Vérification manuelle de la navigation clavier/télécommande sur la nouvelle route (§6, RFC-0004
    §4).

### W14.0 — outillage catalogue

- `theme.schema.json` publié — schéma universel incluant `compatibility` et `profiles` (§7.3),
  validation automatique adaptée par tier (§10.2).
- Template minimal ou outil `create-reefin-theme`.
- Page « Theme Lab » exposant tous les composants `src/ui/` avec les trois thèmes (support de §10.3).
- Registre Git avec approbation manuelle opérationnelle (§10.1, §10.4).
- Galerie officielle : captures, compatibilité affichée, tier et badge « Platform Theme (web) » pour
  les communautaires Web-only (§10.2).
- Installation locale vérifiée par signature (§10.1, §10.3).
- Critères de vérification :
  - Un thème de test tiers passe la validation automatique de bout en bout (§10.3).
  - Installation locale testée manuellement avec vérification de signature effective (échec si
    signature invalide).
  - URL de registre configurée via variable, pas en dur (§10.5).
...
serveur (§10.6, RFC-0003 §12). Non planifié en tranche précise à ce stade.
...
1. **Synchronisation serveur du choix de thème** — ce RFC tranche le maintien en localStorage-only
   pour la v1 (§9.3), mais le contrat anticipe déjà un modèle à deux niveaux (choix de compte +
   override local par appareil, §9.3). Reste ouvert : faut-il et quand pousser le choix de compte via
   `DisplayPreferences` pour un partage multi-appareils — l'override local restant prioritaire s'il
   est défini —, et à quelle tranche (au plus tôt après W14.0) ?
2. **Sort de `purplehaze`** — réécriture complète en préréglage Classic conforme (perte probable de
   fidélité visuelle sur les dégradés/`border-radius` propres, §4.1/§8.1) vs. abandon pur et simple en
...
3. **Outillage catalogue de composants** — Storybook (outil tiers standard, coût d'intégration/
   maintenance) vs. une page « Theme Lab » maison (§11, W14.0) déjà nécessaire pour la validation
   catalogue (§10.3). Un choix pourrait ne pas exclure l'autre selon l'usage (documentation développeur
...
4. **Signature des artefacts** — quel schéma de clés pour signer les paquets de thème du registre
   (§10.1, §10.3) : clé unique du registre, clé par auteur, rotation, révocation de clé compromise. Non
   spécifié ici.
5. **Nom/domaine définitif du site catalogue** — conflit de nom/marque non résolu (§10.5). Le format de
...
6. **Pont SCSS↔MUI pour les breakpoints** — `AppOverrides.scss:1-6` redéclare aujourd'hui les
   breakpoints MUI en dur côté SCSS, sans source commune (§4.2). Solution non spécifiée par ce RFC :
