...
- **Relation** : formalise côté web le plan produit déjà validé par le mainteneur côté serveur
  ...
  `design-tesserafin-api-layer.md` (génération OpenAPI, PR1-3 faites) et `design-web-playback-diagnostics.md`
...
| 3 | Clients obligatoirement générés depuis OpenAPI | Types manuels retapés à la main par fonctionnalité (pattern `features/*/api/types.ts`) | **En cours, largement engagé** — `design-tesserafin-api-layer.md` tranche la question (RFC-0001 §9 Q2) et documente un pipeline de génération complet : PR1 (pipeline `tesserafin-sdk`), PR2 (migration du mirror manuel playback diagnostics vers le généré), PR3 (bascule du point de construction) sont **faites**. Ce point 3 est le point du plan le plus avancé côté web à la date de ce RFC |
...
| 6 | Versionnement conjoint serveur/client avec handshake de compatibilité | `MINIMUM_VERSION` figé sur une constante Jellyfin (`10.10.0`) héritée de `@jellyfin/sdk`, sans lien avec la version réelle du serveur ciblé | Partiellement engagé : `design-tesserafin-api-layer.md` §4.1 corrige déjà le symptôme le plus visible (`MINIMUM_VERSION` dérivé de `x-tesserafin-version`/`SharedVersion.cs` plutôt que de la constante Jellyfin). Il n'existe en revanche **aucune politique de versionnement écrite** côté serveur (`design-tesserafin-api-layer.md` §2.1, §10) — le handshake de compatibilité proprement dit reste à concevoir |
...
### (a) `@jellyfin/sdk` → `tesserafin-sdk` : migration transitoire, pas le point d'arrivée

`design-tesserafin-api-layer.md` documente et a déjà largement exécuté (PR1-3) le remplacement de
`@jellyfin/sdk` par `tesserafin-sdk`, généré depuis la spec OpenAPI actuelle du serveur `reefin`. Ce plan
...
§3 points 1-2). `tesserafin-sdk` n'est donc pas le point d'arrivée du plan — c'est une étape intermédiaire
qui retire déjà la dette de génération manuelle (le vrai problème que réglait
`design-tesserafin-api-layer.md`), en attendant que le contrat serveur lui-même change de forme. Quand
`/api/v1` camelCase existera, `tesserafin-sdk` sera **régénéré sur ce nouveau contrat** avec le même
pipeline (`openapi-generator-cli`/`typescript-axios`, §4.1 de `design-tesserafin-api-layer.md`) — pas
remplacé par un nouvel outil. C'est précisément la promesse de migration mécanique que ce document
pose déjà (§3 de `design-tesserafin-api-layer.md`) : elle s'applique aussi bien à un changement de spec
...
1. Calendrier de la rupture de contrat public (§3 points 1-2, 4-6) : quand `/api/v1` camelCase
   ...
   serveur ? Conditionne directement le moment où `tesserafin-sdk` (§13a) doit être régénéré sur le
