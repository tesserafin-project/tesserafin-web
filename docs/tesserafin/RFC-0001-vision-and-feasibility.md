...
**Processus** : tout breaking change doit être **documenté dans un RFC** avant implémentation — dans `docs/tesserafin/` côté web, en miroir de la pratique déjà en place côté serveur (`docs/pr9x-*.md`, `docs/rfc-di-*.md`). Le RFC doit nommer explicitement le(s) critère(s) invoqué(s) parmi les trois ci-dessus. Un changement qui ne remplit aucun des trois critères reste, par défaut, un changement à faire sans casser la compatibilité — ou à ne pas faire.
