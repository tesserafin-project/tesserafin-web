...
`src/scripts/tesserafinPlaybackCapabilities.ts:321`. This document answers only *whether that is
...
- `VideoRangeTypes: ['SDR']` stays exactly as it is at `src/scripts/tesserafinPlaybackCapabilities.ts:321`.
- Issue #29 stays open.
...
3. **A conjunction function that is deliberately hard to misuse**: HDR10 is claimable only when
   ...
   `HDR10` positive (§4.1).
4. **Wire nothing.** `tesserafinPlaybackCapabilities.ts:321` keeps returning
...
An optional adjunct, **not** done: fold the `docs/tesserafin/bench-hdr/hdr-probe.mjs` script into an
opt-in (`TESSERAFIN_HDR_BENCH=1`) manual script, kept out of the default test run, so the measurement is
...
```bash
node docs/tesserafin/bench-hdr/hdr-probe.mjs \
  ~/.cache/ms-playwright/chromium-1228/chrome-linux64/chrome chromium-149-headless
DISPLAY=:0.0 node docs/tesserafin/bench-hdr/hdr-probe.mjs \
