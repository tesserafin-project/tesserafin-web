...
3. **Optionnel, gain immédiat** : alias npm TS 7.0.2 pour le seul `build:check` (type-check CI 8–12×).
   **Testé lors de l'exécution de l'étape 7 (16/07/2026) : rejeté pour l'instant.** `tsc` 7.0.2 type-check le repo en ~2,9 s (vs ~11–14 s en 6.0.3) mais remonte **~90 erreurs** absentes en 6.0.3 (`TS2554`, `TS2339`, `TS7006`…) — divergence non investiguée (piste : traitement des `.js` JSDoc type `PlaybackManager` par le portage Go). Sortie non identique → alias non activé. Piège pratique relevé : installer l'alias écrase le symlink `node_modules/.bin/tsc` (restaurer via `npm rebuild typescript`). À réévaluer avec TS 7.1.
