...
1. **Une valeur sémantique n'est pas résoluble sans une table de résolution cachée.** `"reduced"`
   ...
   les renderers Web / Compose / SwiftUI que `tesserafin-design/` existe précisément pour unifier
...
`tesserafin-design/themes/glass/` existe, et ces partials se lient désormais à son `tokens.json` réel.
...
`tesserafin-design/web/backdrop-filter.mjs` et est importée à la fois par le générateur (build) et par
