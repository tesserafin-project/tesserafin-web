...
- Le principe directeur d'unification cross-platform du langage visuel et du contrat de thème avec
  les futures apps natives Android/iOS/TV, et le socle `reefin-design` qui le porte (§3).
...
### 3.2 Socle `reefin-design` (nom provisoire)
...
Web ni engager de travail natif prématuré. Le schéma des thèmes de `reefin-design` est le format
...
- Première passe de fixtures de référence : Classic sérialisé dans le format universel `reefin-design`
...
- Retrait de l'import résiduel `@jellyfin/sdk` de `home.tsx:1` (§4.2) au profit de `lib/reefin-sdk`.
