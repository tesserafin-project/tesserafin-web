...
**Status: reconnaissance only. No production code changed. `VideoRangeTypes: ['SDR']` stays. #29 stays open.**
...
measurable here*, and what a probe would have to look like. It ships no detection logic.
...
## 7. Proposed next slice — contains no HDR-claiming code

The slice below can be built and merged with the hardware situation exactly as it is. It changes no
emitted value; it makes the *absence* of detection explicit and machine-checked, which is #29 item 3.

1. **A pure probe module** (e.g. `src/scripts/hdrProbe.ts`) exporting one function that returns a
   tri-state result and is *not wired into `buildDecodeCapabilities`*:
   ```ts
...
2. **Unit tests that exercise all four outcomes** with an injected fake `matchMedia`/`mediaCapabilities`
   — testing the *tri-state logic*, explicitly not asserting any HDR capability of the host. In
...
An optional adjunct: fold the `docs/reefin/bench-hdr/hdr-probe.mjs` script into an opt-in
(`REEFIN_HDR_BENCH=1`) manual script, kept out of the default test run, so the measurement is
