...
- **Il n'existe en revanche aucun document de politique de versionnement API** (recherche dans
  `docs/reefin/*.md` côté serveur : aucun résultat pour « politique de version », « breaking API »,
...
- Nouveau module local **`src/lib/reefin-sdk/`**, sous-dossier `generated/` produit par
...
- Script npm dédié (`generate:reefin-sdk`) qui télécharge la spec, corrige les incompatibilités
...
- Corrige au passage le constat de §2.1 : `MINIMUM_VERSION` doit être dérivé de la version Reefin
  réelle (`x-reefin-version` / `SharedVersion.cs`), pas resté sur la constante Jellyfin `10.10.0`
  héritée de `@jellyfin/sdk`.
- Nom de paquet : `reefin-sdk` en local le temps de la coexistence (pas de publication npm tant
...
- Le seul changement structurel exigé par ce document : son point de sortie vers le SDK typé
  ...
  instance `reefin-sdk` plutôt que `@jellyfin/sdk`, sans changer sa propre logique interne
...
- Le pattern posé par `design-web-playback-diagnostics.md` (`features/playback/api/types.ts` +
  ...
  `reefin-sdk` est régénéré et inclut la route, le mirror manuel est supprimé au profit du client
...
- Cible : une constante unique (ex. `REEFIN_CLIENT_IDENTITY`, nom logique + éventuel nom d'affichage
  séparé) injectée **une fois** au point de construction du SDK (`reefin-sdk`'s `createApi`
...
src/
  lib/
    reefin-sdk/                       # remplace progressivement @jellyfin/sdk
      ...
      versions.ts                     # MINIMUM_VERSION dérivé de x-reefin-version, pas de la constante Jellyfin héritée
    reefin-connection/                 # successeur de lib/jellyfin-apiclient/ (catégorie (c), non généré)
      ConnectionManager.ts
      ServerConnections.ts
      credentials.ts
      websocket.ts
      compat.ts                        # point d'articulation unique : construit l'Api reefin-sdk (§4.4)
  apps/
    dashboard/
      features/
        playback/
          api/                         # PR2 : mirror manuel remplacé par reefin-sdk généré
        <feature>/
          api/                         # nouveau : uniquement filet transitoire (§4.3), pas la norme
  hooks/
    useApi.tsx                         # évolue pour exposer l'Api reefin-sdk (nom de champ à décider en PR3)
scripts/
  generate-reefin-sdk.ts               # script CI/local : télécharge la spec, génère, applique le wrapper
.github/workflows/
  reefin-sdk-contract-check.yml        # PR1 : job openapi-diff web, cf. §6
```
...
1. **Pin de version** : `reefin-sdk/generated/` embarque (ou un fichier `reefin-sdk/spec-version.json`
   à côté) la version/l'horodatage de la spec utilisée pour la dernière génération.
2. **Job CI web dédié** (`reefin-sdk-contract-check.yml`) : télécharge la spec `unstable` (ou
   ...
   `openapi-workflow-run.yml` côté serveur) invitant à régénérer `reefin-sdk`.
3. **Test de fraîcheur du généré** : un script (`verify:reefin-sdk-fresh`) régénère dans un dossier
...
verticale (RFC-0001 §6.5) qui touche une route migre ses propres appels vers `reefin-sdk` /
...
- **Ordre de migration**, aligné sur la roadmap RFC-0001 §7 et sur la structure §1.2/§6.3 du RFC :
  ...
  3. `components/`, `elements/`, `apps/legacy` — catégorie (b) de §2.3 en majorité (méthodes
     ...
     fine).
- **Suivi** : un tableau de progression (fichier markdown simple dans `docs/reefin/`, ou label
...
- **Divergence de spec pendant la fenêtre de régénération** : entre un merge serveur et la prochaine
  régénération web, une route Reefin nouvelle n'est pas encore disponible dans `reefin-sdk` — le
