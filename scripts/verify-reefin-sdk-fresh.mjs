 ...
 * artifact with no provenance at all (see the HISTORY note in generate-reefin-sdk.mjs).
 *
...
function checkProvenance(version) {
    ...
    console.log(
        `[verify:reefin-sdk-fresh] Pinned spec sha256 matches, provenance = ` +
    ...
    if (!serverRepo) {
        console.warn(
            '[verify:reefin-sdk-fresh] WARNING: no reefin server checkout reachable, so the ' +
                ...
                'set $REEFIN_SERVER_REPO to a reefin checkout to enable the full drift check.'
        );
        return true;
    }
    ...
    // Must stay in lockstep with generate-reefin-sdk.mjs's main(): same transforms, same order.
    fixSchema(spec);
    ...
    if (expected !== pinnedText) {
        ...
        fail(
            `${PINNED_SPEC_RELATIVE} has DRIFTED from the canonical contract at ` +
                ...
                'Re-run `npm run generate:reefin-sdk` and commit the result.'
        );
        return false;
    }
    console.log(
        '[verify:reefin-sdk-fresh] Pinned spec matches the canonical contract at ' +
    ...
    try {
        ...
        if (master !== version.sourceCommit) {
            ...
            console.log(
                `[verify:reefin-sdk-fresh] NOTE: pinned contract is ${behind} commit(s) behind ` +
